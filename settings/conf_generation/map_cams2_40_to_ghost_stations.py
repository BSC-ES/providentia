...
def cross_check(stations_unfound, ghost_stations, associated_networks, mapped_stations):
    ...
    for stn in stations_unfound:
        ...
        if stn not in found_stations:
            # associated networks
            for associated_network_ii, associated_network in enumerate(
                associated_networks
            ):
...
def create_config_file(
    fname, section, network, resolution, species, start_date, end_date, mapped_stations
):
    mapped_stations_str = ", ".join(
        [str(elem) for elem in sorted(list(mapped_stations.values()))]
    )
...
if process_type == "IRA":
    station_files = sorted(
        glob.glob(
            "/esarchive/obs/ineris/eionet-cams2_40-ira/original_files/stations_{}/stations*".format(
                station_year
            )
        )
    )
    ...
    station_files = sorted(
        glob.glob(
            "/esarchive/obs/ineris/eionet-cams2_40-vra/original_files/stations_{}/stations*".format(
                station_year
            )
        )
    )
...
for speci in species:
    station_files_cut = [
        ...
        if ".{}.".format(speci) in station_file
    ]
    ...
    for station_file in station_files_cut:
        ...
        conf_fname = "ecmwf_cams2_40_{}_{}_{}_background_{}_{}_{}.conf".format(
            speci, process_type, eval_type, setno, start_date, end_date
        )

        # get relevant ghost files
        ghost_files = sorted(
            glob.glob(
                "/gpfs/projects/bsc32/AC_cache/obs/ghost/{}/{}/{}/{}/{}*".format(
                    network, ghost_version, resolution, speci_mapped, speci_mapped
                )
            )
        )
        ...
        for ghost_file in ghost_files:
            ...
            stations_unfound, mapped_stations = cross_check(
                stations_unfound,
                ghost_station_references,
                ghost_associated_networks,
                mapped_stations,
            )

            # replace turkey codes
            stations_unfound = [
                "TR{}".format(stn[1:]) if (stn[0] == "T") & (stn[1].isdigit()) else stn
                for stn in stations_unfound
            ]
            stations_unfound, mapped_stations = cross_check(
                stations_unfound,
