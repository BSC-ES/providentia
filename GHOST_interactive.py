...
class generate_GHOST_interactive_dashboard(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.cb_z1.setMinimumWidth(130)
        self.cb_z1.setMaximumWidth(130)
        self.cb_z2 = QtWidgets.QComboBox(self)
        self.cb_z2.setMinimumWidth(130)
        self.cb_z2.setMaximumWidth(130)
        self.vertical_splitter_3 = QVLine()
        self.vertical_splitter_3.setMaximumWidth(20)
        self.lb_model_bias = QtWidgets.QLabel(self, text = "Exp. Bias")
        self.lb_model_bias.setFont(title_font)
        self.cb_model_bias_type = QtWidgets.QComboBox(self)
        self.cb_model_bias_type.setMinimumWidth(80)
        self.cb_model_bias_type.setMaximumWidth(80)
        self.cb_model_bias_stat = QtWidgets.QComboBox(self)
        self.cb_model_bias_stat.setMinimumWidth(80)
        self.cb_model_bias_stat.setMaximumWidth(80)
        ...
        config_bar.addWidget(self.lb_model_bias, 0, 12, QtCore.Qt.AlignCenter)
        config_bar.addWidget(self.cb_model_bias_type, 1, 12)
        config_bar.addWidget(self.cb_model_bias_stat, 2, 12)
        ...
        self.flag_codes = np.sort(standard_data_flag_codes.values()) 
        ...
        self.qa_codes = np.sort(standard_qa_flag_codes.values())        
        ...
        self.classification_codes = np.sort(standard_classification_flag_codes.values())
    ...
    def update_configuration_bar_fields(self):
        ...
        for network in available_networks:
            ...
            for resolution in available_resolutions:
                ...
                if len(valid_species) != 0:
                    ...
                    if network not in self.available_observation_data.keys():
                    ...
                    if resolution not in self.available_observation_data[network].keys():
                ...
                for resolution in available_resolutions:
                    ...
                    for species in available_species:
                        ...
                        for network in valid_networks:
                            ...
                            #check if have written specific resolution previously to data dictionary, if not write it   
                            if resolution not in self.available_experiment_data.keys():
                            ...
                            if species not in self.available_experiment_data[resolution].keys():
                                self.available_experiment_data[resolution][species] = {}
                            
                            #for each valid network associate current experiment-grid string name
                            if network not in self.available_experiment_data[resolution][species].keys():
        ...
        available_networks = sorted(self.available_observation_data.keys())
        ...
        available_resolutions = self.available_observation_data[self.cb_network.currentText()].keys()
    ...
    def handle_data_selection_update(self):
        ...
        if (read_all == True) or (read_left == True) or (read_right == True) or (cut_left == True) or (cut_right == True):         
            ...
            if read_all == True:
                ...
                if np.array_equal(self.previous_station_references, self.station_references) == False:  
                    ...
                    for data_label in self.data_in_memory.keys():
        ...
        #--------------------------------------------------------------------#
        #check if have valid stations left after data filtering, if not return here and do not update canvas.
        #if len(self.data_in_memory_filtered['observations']['valid_station_inds']) == 0:
        #    return
        ...
        self.basic_z_stats = np.array(OrderedDict(sorted(basic_stats_dict.iteritems(), key=lambda x: x[1]['order'])).keys())
        self.basic_and_bias_z_stats = np.append(self.basic_z_stats, OrderedDict(sorted(experiment_bias_stats_dict.iteritems(), key=lambda x: x[1]['order'])).keys())
        ...
        if len(self.data_in_memory.keys()) == 1:
            self.z1_arrays = np.array(['observations'])
        else:
            data_array_labels = np.array(self.data_in_memory.keys())
    ...
    def read_setup(self):
        ...
        #make some key variables globally accessible (for access by parallel netCDF reading functions)
        __builtin__.time_array = self.time_array
        __builtin__.active_species = self.active_species
        __builtin__.selected_qa = self.qa_codes[self.active_qa_inds]
        __builtin__.selected_flags = self.flag_codes[self.active_flag_inds]        
        __builtin__.selected_classifications_to_retain = self.classification_codes[self.active_classifications_to_retain_inds]    
        __builtin__.selected_classifications_to_remove = self.classification_codes[self.active_classifications_to_remove_inds]  
        ...
        #define arrays to store all station references, longitudes, latitudes, measurement methods from each read file
        all_station_references = np.array([], dtype=np.object)
        all_station_longitudes = np.array([], dtype=np.float32)
        all_station_latitudes = np.array([], dtype=np.float32)
        all_station_measurement_methods = np.array([], dtype=np.object)

        #read serially
        if self.read_type == 'serial':
            #iterate through relevant files
            for relevant_file_ii, relevant_file in enumerate(relevant_files):
                file_station_references, file_station_longitudes, file_station_latitudes, file_station_measurement_methods = read_netCDF_station_information(relevant_file)
                all_station_references = np.append(all_station_references, file_station_references)
                all_station_longitudes = np.append(all_station_longitudes, file_station_longitudes)
                all_station_latitudes = np.append(all_station_latitudes, file_station_latitudes)
                all_station_measurement_methods = np.append(all_station_measurement_methods, file_station_measurement_methods)
        ...
        #read in parallel (utilising all available CPUs)
            ...
            all_file_data = pool.map(read_netCDF_station_information, relevant_files)
            ...
            for relevant_file_ii, file_data in enumerate(all_file_data):
                file_station_references, file_station_longitudes, file_station_latitudes, file_station_measurement_methods = file_data[0], file_data[1], file_data[2], file_data[3] 
                all_station_references = np.append(all_station_references, file_station_references)
                all_station_longitudes = np.append(all_station_longitudes, file_station_longitudes)
                all_station_latitudes = np.append(all_station_latitudes, file_station_latitudes)
                all_station_measurement_methods = np.append(all_station_measurement_methods, file_station_measurement_methods)
                
        #get unique sorted station references across all files --> write to self
        self.station_references, unique_indices = np.unique(all_station_references, return_index=True)
        #get longitudes/latitudes/measurement methods associated with unique station references --> also write to self
        self.station_longitudes = all_station_longitudes[unique_indices]
        self.station_latitudes = all_station_latitudes[unique_indices]
        self.station_measurement_methods = all_station_measurement_methods[unique_indices]
        ...
        #make station references variable globally accessible (for access by parallel netCDF reading functions)
        __builtin__.station_references = self.station_references
    ...
    def read_data(self, data_label, start_date_to_read, end_date_to_read):
        ...
        __builtin__.process_type = process_type
        ...
        #if files are given in monthly chunked files, add '01' string to date 
        relevant_file_start_dates = [int(relevant_file_start_date+'01') if len(relevant_file_start_date) == 6 else int(relevant_file_start_date) for relevant_file_start_date in relevant_file_start_dates] 
        first_valid_file_ind = bisect.bisect_right(relevant_file_start_dates, int(start_date_to_read))-1
        ...
        if data_label not in self.data_in_memory.keys():  
    ...
    def update_plotting_parameters(self):
        ...
        sns.reset_orig()
        clrs = sns.color_palette('husl', n_colors=len(self.data_in_memory.keys())-1)

        #iterate through sorted experiment names, assigning each experiment a new RGB colour tuple, and zorder
        for experiment_ind, experiment in enumerate(sorted(self.data_in_memory.keys())):
            if experiment != 'observations':
                #define colour for experiment
                self.data_in_memory[experiment]['colour'] = clrs[experiment_ind]
                #define zorder for experiment (5+experiment_ind)
                self.data_in_memory[experiment]['zorder'] = 5+experiment_ind

#--------------------------------------------------------------------------------#
#--------------------------------------------------------------------------------#
#function that handles reading of observational station references/longitudes/latitudes/measurement methods/measurement units in a netCDF file 
...
def read_netCDF_station_information(relevant_file):
    ...
    #get all station references/longitudes/latitudes in file
    file_station_references = nCDF_root['station_reference'][:]
    file_station_longitudes = nCDF_root['longitude'][:]  
    file_station_latitudes = nCDF_root['latitude'][:]
    file_station_measurement_methods = nCDF_root['measurement_methodology'][:]

    #close netCDF
    nCDF_root.close()

    return file_station_references, file_station_longitudes, file_station_latitudes, file_station_measurement_methods
...
def read_netCDF_data(relevant_file):
    ...
    #get file time
    file_time = num2date(nCDF_root['time'][:], nCDF_root['time'].units)
    #remove microseconds
...
class MPL_Canvas(FigureCanvas):
    ...
    def __init__(self, read_instance, parent=None):
        ...
        gridspec.update(left=0.00,right=0.99,top=0.96,bottom=0.02, wspace=0.00, hspace=0.00)
        
        map_ax =             gridspec.new_subplotspec((0, 0),   rowspan=46, colspan=46)
        legend_ax =          gridspec.new_subplotspec((0, 46),  rowspan=8,  colspan=54)
        ts_ax =              gridspec.new_subplotspec((12, 54), rowspan=35, colspan=46)
        violin_hours_ax =    gridspec.new_subplotspec((52, 70), rowspan=21, colspan=30)    
        violin_months_ax =   gridspec.new_subplotspec((77, 70), rowspan=21, colspan=18)
        violin_days_ax =     gridspec.new_subplotspec((77, 91), rowspan=21, colspan=9)
        #exp_bias_hours_ax =  gridspec.new_subplotspec((52, 35), rowspan=21, colspan=30)    
        #exp_bias_months_ax = gridspec.new_subplotspec((77, 35), rowspan=21, colspan=18)
        #exp_bias_days_ax =   gridspec.new_subplotspec((77, 56), rowspan=21, colspan=9)
        ...
        #map colorbar ax 
        self.cbar_ax = self.figure.add_axes([0.03, 0.516, 0.34, 0.008])

        #--------------------------------------------------#
        #adjust specifics of map axis - turning off spines
        self.map_ax.spines["top"].set_visible(False)      
        self.map_ax.spines["bottom"].set_visible(False)
        self.map_ax.spines["left"].set_visible(False)
        self.map_ax.spines["right"].set_visible(False)
        
        #--------------------------------------------------# 
        #adjust specifics of time series axis - turning off spines
    ...
    def update_MPL_canvas(self):
        ...
        #reset list of active violin plot axes (which varies by temporal resolution of data)
        self.active_violin_axes = []
        
        #if does not yet exist, setup empty basemap map on axis
        try:
            self.bm
        except:
            #setup basemap map using Eckert IV Projection
            self.bm = Basemap(projection='eck4',lon_0=0,resolution='c', ax=self.map_ax)
            #draw coastlines
            self.bm.drawcoastlines()
            #draw parallels and meridians.
            self.bm.drawparallels(np.arange(-90.,120.,30.),color='lightgrey',alpha=0.8)
            self.bm.drawmeridians(np.arange(0.,360.,60.),color='lightgrey',alpha=0.8)

        #reset variable of selected station indices on map as empty list
        self.selected_station_inds = np.array([],dtype=np.int) 
        #clear previously selected station points 
        try:
            self.selected_map_points.pop(0).remove()        
        except:
            pass  
    ...
    def handle_data_filter_update(self):
        ...
        for data_label in self.read_instance.data_in_memory_filtered.keys(): 
    ...
    def colocate_data(self):
        ...
        if len(self.read_instance.data_in_memory.keys()) == 1:
            self.colocate_active = False
            return
        else:
            ...
            for data_label in self.read_instance.data_in_memory.keys():
                if data_label != 'observations':
                    ...
                    self.read_instance.data_in_memory_filtered['observations_colocatedto_%s'%(data_label)] = {'data':obs_data} 
                    ...
                    self.read_instance.data_in_memory_filtered['%s_colocatedto_observations'%(data_label)] = {'data':exp_data}
            ...
            self.read_instance.data_in_memory_filtered['observations_colocatedto_experiments'] = {'data':obs_data}   
    ...
    def handle_colocate_update(self):
        ...
        if self.read_instance.block_MPL_canvas_updates == False:
            ...
            if len(self.read_instance.data_in_memory_filtered.keys()) == 1:
                self.colocate_active = False
                return
            ...
            #update plotted map z statistic
            self.update_map_z_statisitic()
            #update map station selection (and associated plots)
    ...
    def update_map_z_statisitic(self):
        ...
        #if have no valid active map indices, clear all previously plotted/selected map points, the colourbar, and update title
        if len(self.active_map_valid_station_inds) == 0:
            ...
            self.cbar_ax.cla()
            self.cbar_ax.axis('off')
            #clear previously selected station indices
            self.selected_station_inds = np.array([],dtype=np.int)
            self.update_map_station_selection()

        #otherwise plot valid active map indices
        else:
            #compute map projection coordinates for each pair of valid active map longitude/latitude coordinates
            x,y=self.bm(self.read_instance.station_longitudes[self.active_map_valid_station_inds],self.read_instance.station_latitudes[self.active_map_valid_station_inds])
            ...
            if np.all(np.in1d(self.selected_station_inds, self.active_map_valid_station_inds)) == False:
                #reset self.selected_station_inds to be empty list, and unselect stations
                self.selected_station_inds = np.array([],dtype=np.int)
                #update map selection
                self.update_map_station_selection()
            else:
                #otherwise, simply update map title
                if len(self.selected_station_inds) == 1:
                    self.map_ax.set_title('%s Selected'%(self.read_instance.station_references[self.selected_station_inds][0]), fontsize=9.5, weight='light', pad=3)
                else:
                    self.map_ax.set_title('%s Stations Selected of %s Available'%(len(self.selected_station_inds), len(self.active_map_valid_station_inds)), fontsize=9.5, weight='light', pad=3)
            ...
            self.map_points = self.map_ax.scatter(x, y, s=6, c=self.z_statistic, vmin=self.z_vmin, vmax=self.z_vmax, cmap=self.z_colourmap, picker = 1, zorder=2)     

            #colourbar
            #generate colourbar tick array 
            tick_array = np.linspace(self.z_vmin, self.z_vmax, 5, endpoint=True)
            #plot colourbar
            cb = self.figure.colorbar(self.map_points, orientation = 'horizontal', cax=self.cbar_ax, label='', ticks=tick_array)
            self.cbar_ax.tick_params(labelsize=8.5)
            #plot colourbar label
            self.cbar_ax.yaxis.set_label_position("right")
            self.cbar_ax.set_ylabel(self.z_label, fontsize=8.3, weight='light', rotation=0, ha='left',va='center')
            #turn colourbar axis on
            self.cbar_ax.axis('on')

    #--------------------------------------------------------------------------------#
    #--------------------------------------------------------------------------------# 
    #function that updates the selection of stations on map
    #associated time series plots, violin plots, experiment bias plots are also updated 
    ...
    def update_map_station_selection(self):
        ...
        #clear/hide time series/violin plots/experiment bias axes
        #clear axes
        self.ts_ax.cla()
        for ax in self.active_violin_axes:
            ax.cla()
        #hide axes
        self.ts_ax.axis('off')
        for ax in self.active_violin_axes:
        ...
        #update map plot (if have valid stations)
        if len(self.selected_station_inds) > 0:
    
            #compute map projection coordinates for each pair of longitude/latitude coordinates
            x,y=self.bm(self.read_instance.station_longitudes[self.selected_station_inds],self.read_instance.station_latitudes[self.selected_station_inds])
            self.selected_map_points = self.map_ax.plot(x, y, 'o', ms=4.2, alpha=1.0, color='hotpink', zorder=3, markerfacecolor='none', markeredgewidth=0.8)

            #put selected data into pandas dataframes
    ...
    def update_experiment_grid_domain_edges(self):
        ...
        for experiment in sorted(self.read_instance.data_in_memory.keys()):
            if experiment != 'observations':
                ...
                grid_edge_outline_poly = Polygon(np.vstack((exp_x,exp_y)).T, edgecolor=self.read_instance.data_in_memory[experiment]['colour'], linewidth=1, linestyle='--', fill=False, zorder=1)
    ...
    def update_legend(self):
        ...
        for experiment_ind, experiment in enumerate(sorted(self.read_instance.data_in_memory.keys())):
            if experiment != 'observations':
    ...
    def to_pandas_dataframe(self):
        ...
        #iterate through data arrays in data in memory dictionary
        for data_label in self.read_instance.data_in_memory.keys():   

            #define data array to use (i.e. colocated or not)
            #observational arrays
            if data_label == 'observations':
                if self.colocate_active == True:
                    data_array = self.read_instance.data_in_memory_filtered['observations_colocatedto_experiments']['data'][:,self.selected_station_inds]
                else:
                    data_array = self.read_instance.data_in_memory_filtered['observations']['data'][:,self.selected_station_inds]
            #experiment arrays
            else:
                if self.colocate_active == True:
                    data_array_name = '%s_colocatedto_observations'%(data_label)
                else:
                    data_array_name = data_label            
                #get intersect between selected station indices and valid available indices for experiment data array
                valid_selected_station_indices = np.intersect1d(self.selected_station_inds, self.read_instance.data_in_memory_filtered[data_array_name]['valid_station_inds'])
                #get data
                data_array = self.read_instance.data_in_memory_filtered[data_array_name]['data'][:,valid_selected_station_indices]

            #if data array has no valid data for selected stations, do not create a pandas dataframe, rather delete the previously created dataframe for data array -- if exists (can happen when all selected stations are outside experiment domain extents)
            
            #data array has valid data?
            if data_array.size:
                #put data array into a pandas dataframe -->  add to data in memory dictionary
                self.read_instance.data_in_memory_filtered[data_label]['pandas_df'] = pd.DataFrame(np.nanmean(data_array, axis=1), index=self.read_instance.time_array, columns=['data'])
            #data array has no valid data?
            else:
                #remove previously created dataframe for data array (if exists)
                if 'pandas_df' in self.read_instance.data_in_memory_filtered[data_label].keys():
                    del self.read_instance.data_in_memory_filtered[data_label]['pandas_df']
    #--------------------------------------------------------------------------------#
    #--------------------------------------------------------------------------------# 
    ...
    def update_time_series_plot(self):
        ...
        for data_label in self.read_instance.data_in_memory.keys():   
            ...
            #check if data array has an associated pandas dataframe (i.e. some valid data), if not continue to next array
            if 'pandas_df' in self.read_instance.data_in_memory_filtered[data_label].keys():

                #plot observational time series data
                self.data_array_ts = self.ts_ax.plot(self.read_instance.data_in_memory_filtered[data_label]['pandas_df'], color=self.read_instance.data_in_memory[data_label]['colour'], marker = 'o', markeredgecolor = None, mew = 0, markersize = 2, linestyle = 'None', zorder=self.read_instance.data_in_memory[data_label]['zorder'])
                    
        #set axes labels
        self.ts_ax.set_ylabel('Concentration (%s)'%(self.read_instance.measurement_units), fontsize=8.5, weight='light')

        #plot grid
        self.ts_ax.grid(color='lightgrey',alpha=0.8)

        #set axis tick label sizes
        self.ts_ax.tick_params(labelsize=8.5)

    #--------------------------------------------------------------------------------#
    #--------------------------------------------------------------------------------# 
    #function that updates temporal violin plots upon selection of station/s

    def update_violin_plots(self):

        #redefine active violin plot axes by the temporal resolution of the data
        if self.read_instance.active_resolution == 'hourly':
            self.active_violin_axes = [self.violin_hours_ax, self.violin_days_ax, self.violin_months_ax]
            active_violin_titles = ['H', 'DoW', 'M']        
        elif self.read_instance.active_resolution == 'daily':
            self.active_violin_axes = [self.violin_days_ax, self.violin_months_ax]
            active_violin_titles = ['DoW', 'M']
        elif self.read_instance.active_resolution == 'monthly':
            self.active_violin_axes = [self.violin_months_ax]
            active_violin_titles = ['M']

        #turn on active axes and add yaxis grid, and change axis label tick sizes
        for ax_ii, ax in enumerate(self.active_violin_axes):
            ax.axis('on')   
            ax.yaxis.grid(color='lightgrey',alpha=0.8)
            ax.annotate(active_violin_titles[ax_ii], (0, 1), xytext=(2, -2), xycoords='axes fraction', textcoords='offset points', fontsize=9, fontweight='light', ha='left', va='top')
            ax.tick_params(labelsize=8.5)

        #------------------------------------------------------------------------------------------------#

        #calculate hourly/monthly/day of week averages, then make viloin plots  
        
        #create dictionary for storing violin plot objects made (for observations and experiments), medians and associated xticks
        violin_plots_dict = {}
        for data_label in self.read_instance.data_in_memory_filtered.keys():   
            violin_plots_dict[data_label] = {'plots':[], 'medians':[], 'xticks':[]}

        #---------------------------------------------------#
            
        #make hour of day violin plot

        if self.read_instance.active_resolution == 'hourly':
              
            #create array for storing all hours at which a violin distribution has been plotted
            all_hours = []
            #create arrays for storing lower/upper percentiles (p5 and p95) of violin distributions for later limiting ylim 
            all_lower_pc = []
            all_upper_pc =  []
            ...
            for data_label in self.read_instance.data_in_memory.keys():   
                ...
                if 'pandas_df' in self.read_instance.data_in_memory_filtered[data_label].keys():
                    ...
                    #group data array into hourly chunks
                    grouped_hourly_data = [g['data'].dropna() for n, g in self.read_instance.data_in_memory_filtered[data_label]['pandas_df'].groupby(self.read_instance.data_in_memory_filtered[data_label]['pandas_df'].index.hour)]
                    #drop any hourly chunks which have no data
                    grouped_hourly_data = [grouped_hour_data for grouped_hour_data in grouped_hourly_data if len(grouped_hour_data) != 0]
                    #get list of all hours there is valid data
                    hours = [grouped_hour_data.index[0].hour for grouped_hour_data in grouped_hourly_data]
                    all_hours = np.append(all_hours, hours)
                    #make violin plots
                    violin_plots_dict[data_label]['plots'].append(self.violin_hours_ax.violinplot(grouped_hourly_data, positions=hours, points=100, widths=0.85, showmeans=False, showmedians=False, showextrema=False))
                    #save medians by hour
                    violin_plots_dict[data_label]['medians'].append([grouped_hour_data.quantile(0.5) for grouped_hour_data in grouped_hourly_data]) 
                    #save hours have plotted violin distributions at for later use
                    violin_plots_dict[data_label]['xticks'].append(hours) 
                    #save p5/p95 by hour
                    all_lower_pc=np.append(all_lower_pc, [grouped_hour_data.quantile(0.05) for grouped_hour_data in grouped_hourly_data]) 
                    all_upper_pc=np.append(all_upper_pc, [grouped_hour_data.quantile(0.95) for grouped_hour_data in grouped_hourly_data]) 

            #get all unique hours that a violin distribution has been made at
            unique_hours = np.unique(all_hours)
            #set x axis limits
            self.violin_hours_ax.set_xlim(np.min(unique_hours)-0.5, np.max(unique_hours)+0.5)
            #set y axis limits (as minimum p5, and maximum p95)
            self.violin_hours_ax.set_ylim(np.min(all_lower_pc), np.max(all_upper_pc))
            #set x axis ticks (every 3 hours)
            self.violin_hours_ax.set_xticks(unique_hours[::3])

        #---------------------------------------------------#

        #make day of week violin plot

        if (self.read_instance.active_resolution == 'hourly') or (self.read_instance.active_resolution == 'daily'):

            #create array for storing all daysofweek at which a violin distribution has been plotted
            all_daysofweek = []
            #create arrays for storing lower/upper percentiles (p5 and p95) of violin distributions for later limiting ylim 
            all_lower_pc = []
            all_upper_pc = []
            ...
            for data_label in self.read_instance.data_in_memory.keys():          
                ...
                if 'pandas_df' in self.read_instance.data_in_memory_filtered[data_label].keys():
                    ...
                    #group data array into dayofweek chunks
                    grouped_daysofweek_data = [g['data'].dropna() for n, g in self.read_instance.data_in_memory_filtered[data_label]['pandas_df'].groupby(self.read_instance.data_in_memory_filtered[data_label]['pandas_df'].index.dayofweek)]
                    #drop any dayofweek chunks which have no data
                    grouped_daysofweek_data = [grouped_dayofweek_data for grouped_dayofweek_data in grouped_daysofweek_data if len(grouped_dayofweek_data) != 0]
                    #get list of all daysofweek there is valid data
                    daysofweek = [grouped_dayofweek_data.index[0].dayofweek for grouped_dayofweek_data in grouped_daysofweek_data]
                    all_daysofweek = np.append(all_daysofweek, daysofweek)
                    #make violin plots
                    violin_plots_dict[data_label]['plots'].append(self.violin_days_ax.violinplot(grouped_daysofweek_data, positions=daysofweek, points=100, widths=0.85, showmeans=False, showmedians=False, showextrema=False))
                    #save medians by dayofweek
                    violin_plots_dict[data_label]['medians'].append([grouped_dayofweek_data.quantile(0.5) for grouped_dayofweek_data in grouped_daysofweek_data]) 
                    #save daysofweek have plotted violin distributions at for later use
                    violin_plots_dict[data_label]['xticks'].append(daysofweek) 
                    #save p5/p95 by dayofweek
                    all_lower_pc=np.append(all_lower_pc, [grouped_dayofweek_data.quantile(0.05) for grouped_dayofweek_data in grouped_daysofweek_data])
                    all_upper_pc=np.append(all_upper_pc, [grouped_dayofweek_data.quantile(0.95) for grouped_dayofweek_data in grouped_daysofweek_data])

            #get all unique daysofweek that a violin distribution has been made at
            unique_daysofweek = np.unique(all_daysofweek)
            #set x axis limits
            self.violin_days_ax.set_xlim(np.min(unique_daysofweek)-0.5, np.max(unique_daysofweek)+0.5)
            #set y axis limits (as minimum p5, and maximum p95)
            self.violin_days_ax.set_ylim(np.min(all_lower_pc), np.max(all_upper_pc))
            #set x axis ticks
            self.violin_days_ax.set_xticks(unique_daysofweek)
            #set x axis tick labels
            self.violin_days_ax.set_xticklabels([temporal_axis_mapping_dict['days_of_week'][dayofweek] for dayofweek in unique_daysofweek])

        #---------------------------------------------------#

        #make monthly violin plot

        #create array for storing all months at which a violin distribution has been plotted
        all_months = []
        #create arrays for storing lower/upper percentiles (p5 and p95) of violin distributions for later limiting ylim 
        all_lower_pc = []
        all_upper_pc =  []        
        ...
        for data_label in self.read_instance.data_in_memory.keys():   
            ...
            if 'pandas_df' in self.read_instance.data_in_memory_filtered[data_label].keys():
                ...
                #group data array into monthly chunks
                grouped_monthly_data = [g['data'].dropna() for n, g in self.read_instance.data_in_memory_filtered[data_label]['pandas_df'].groupby(self.read_instance.data_in_memory_filtered[data_label]['pandas_df'].index.month)]  
                #drop any monthly chunks which have no data
                grouped_monthly_data = [grouped_month_data for grouped_month_data in grouped_monthly_data if len(grouped_month_data) != 0]
                #get list of all months there is valid data
                months = [grouped_month_data.index[0].month for grouped_month_data in grouped_monthly_data]
                all_months = np.append(all_months, months)
                #make violin plots
                violin_plots_dict[data_label]['plots'].append(self.violin_months_ax.violinplot(grouped_monthly_data, positions=months, points=100, widths=0.85, showmeans=False, showmedians=False, showextrema=False))
                #save medians by month
                violin_plots_dict[data_label]['medians'].append([grouped_month_data.quantile(0.5) for grouped_month_data in grouped_monthly_data]) 
                #save months have plotted violin distributions at for later use
                violin_plots_dict[data_label]['xticks'].append(months) 
                #save p5/p95 by month
                all_lower_pc=np.append(all_lower_pc, [grouped_month_data.quantile(0.05) for grouped_month_data in grouped_monthly_data]) 
                all_upper_pc=np.append(all_upper_pc, [grouped_month_data.quantile(0.95) for grouped_month_data in grouped_monthly_data]) 
   
        #get all unique months that a violin distribution has been made at
        unique_months = np.unique(all_months)
        #set x axis limits
        self.violin_months_ax.set_xlim(np.min(unique_months)-0.5, np.max(unique_months)+0.5)
        #set y axis limits (as minimum p5, and maximum p95)
        self.violin_months_ax.set_ylim(np.min(all_lower_pc), np.max(all_upper_pc))   
        #set x axis ticks   
        self.violin_months_ax.set_xticks(unique_months)
        #set x axis tick labels
        self.violin_months_ax.set_xticklabels([temporal_axis_mapping_dict['months'][month] for month in unique_months])

        #------------------------------------------------------------------------------------------------#

        #format violin plots as desired
            
        #iterate through data labels in violin plots dictionary
        for data_label in violin_plots_dict.keys():
           #iterate through list of violin plots made for specific observations/experiment
            for violin_ii, violin_plot in enumerate(violin_plots_dict[data_label]['plots']):
                #get relevant axis data will plotted on
                ax = self.active_violin_axes[violin_ii]
                #update plotted objects with necessary colour, zorder and alpha
                for patch in violin_plot['bodies']:
                    patch.set_facecolor(self.read_instance.data_in_memory[data_label]['colour'])
                    patch.set_zorder(self.read_instance.data_in_memory[data_label]['zorder'])
                    if data_label == 'observations':                  
                        patch.set_alpha(0.7)
                    else:
                        patch.set_alpha(0.5)
                    #if have experiments, split the violin plot across the horizontal (observations on left, experiments on right) 
                    if len(violin_plots_dict.keys()) > 1:
                ...
                median_zorder = (self.read_instance.data_in_memory['observations']['zorder']+len(violin_plots_dict.keys()) - 1) + self.read_instance.data_in_memory[data_label]['zorder']

                #get xticks and medians to plot
                xticks = violin_plots_dict[data_label]['xticks'][violin_ii]
                medians = violin_plots_dict[data_label]['medians'][violin_ii]
                ...
                if len(inds_to_split) == 0:
                    ax.plot(xticks, medians, marker='o', color=self.read_instance.data_in_memory[data_label]['colour'], markersize=3, linewidth=0.5, zorder=median_zorder)
                else:
                    inds_to_split += 1
                    start_ind = 0
                    for end_ind in inds_to_split:
                        ax.plot(xticks[start_ind:end_ind], medians[start_ind:end_ind], marker='o', color=self.read_instance.data_in_memory[data_label]['colour'], markersize=3, linewidth=0.5, zorder=median_zorder)
                        start_ind = end_ind   
                    ax.plot(xticks[start_ind:], medians[start_ind:], marker='o', color=self.read_instance.data_in_memory[data_label]['colour'], markersize=3, linewidth=0.5, zorder=median_zorder) 

        #------------------------------------------------------------------------------------------------#
        #plot yaxis units label
            
        #if selected data resolution is 'hourly', plot the units hung off the axis (hourly or monthly) with the longest string ytick
        if self.read_instance.active_resolution == 'hourly':
            hourly_max_ytick = np.max([len(str(ytick)) for ytick in self.violin_hours_ax.get_yticks()])
            monthly_max_ytick = hourly_max_ytick = np.max([len(str(ytick)) for ytick in self.violin_months_ax.get_yticks()])
            if hourly_max_ytick >= monthly_max_ytick:
                self.violin_hours_ax.set_ylabel('Concentration (%s)'%(self.read_instance.measurement_units), fontsize=8.5, weight='light', ha='right') 
            else:
                self.violin_months_ax.set_ylabel('Concentration (%s)'%(self.read_instance.measurement_units), fontsize=8.5, weight='light', ha='left')
        #otherwise, plot the units on the monthly axis
        else:
            self.violin_months_ax.set_ylabel('Concentration (%s)'%(self.read_instance.measurement_units), fontsize=8.5, weight='light') 
    ...
    def calculate_z_statistic(self):
        ...
        z1_array_name = self.read_instance.cb_z1.currentText()     
        z2_array_name = self.read_instance.cb_z2.currentText()
        ...
        if z_statistic_name in basic_stats_dict.keys():
        ...
        if 'colourbar' in stats_dict.keys():
        ...
        if have_z2 == False:
            self.z_label = '%s\n%s %s'%(z1_array_name, stats_dict['label'], label_units)
        else:
            self.z_label = '%s - %s\n%s %s'%(z2_array_name, z1_array_name, stats_dict['label'], label_units)

        #-------------------------------------------------#
            
        #if colocation is active OR z statistic is a bias statistic, modify z1/z2 array names accordingly to get colocated data arrays
        if (self.colocate_active == True) or (z_statistic_type == 'bias'):      
            #don't have z2 array?
            if have_z2 == False:            
                if z1_array_name == 'observations':
                    z1_array_name = 'observations_colocatedto_experiments'
                else:
                    z1_array_name = '%s_colocatedto_observations'%(z1_array_name)
            #have z2 array? 
            elif have_z2 == True:
                if z1_array_name == 'observations':
                    z1_array_name = 'observations_colocatedto_%s'%(z2_array_name)
                else:
                    z1_array_name = '%s_colocatedto_observations'%(z1_array_name)       

                if z2_array_name == 'observations':
                    z2_array_name = 'observations_colocatedto_%s'%(z1_array_name)
                else:
                    z2_array_name = '%s_colocatedto_observations'%(z2_array_name)   
        ...
        if have_z2 == False:
            self.active_map_valid_station_inds = self.read_instance.data_in_memory_filtered[z1_array_name]['valid_station_inds']
        else:
            #if have z2 array, get intersection of z1 and z2 valid station indices
            self.active_map_valid_station_inds = np.intersect1d(self.read_instance.data_in_memory_filtered[z1_array_name]['valid_station_inds'], self.read_instance.data_in_memory_filtered[z2_array_name]['valid_station_inds'])

        #-------------------------------------------------#

        #read z1 data
        z1_array_data = self.read_instance.data_in_memory_filtered[z1_array_name]['data'][:,self.active_map_valid_station_inds]
        ...
        if have_z2 == False:
            ...
            #read z2 data 
            z2_array_data = self.read_instance.data_in_memory_filtered[z2_array_name]['data'][:,self.active_map_valid_station_inds]
            #drop NaNs and reshape to object list of station data arrays (if not checking data %)
            if z_statistic_name != 'Data %':
    ...
    def handle_map_z_statistic_update(self):
        ...
        if self.read_instance.block_config_bar_handling_updates == False:
            ...
            #force them to be 'observations' and first basic z statisitic respecively
            if selected_z_stat == '':
            ...
            #if have a z2 array selected (i.e. not empty string), make sure bias statistics are set in z statistic items
            if selected_z2_array !=  '':
                z_stat_items = self.read_instance.basic_and_bias_z_stats 
            #else, if do not have a z2 array selected (i.e. empty string), then make sure bias statistics are not available in z statistic items 
            else:
                z_stat_items = self.read_instance.basic_z_stats 
    ...
    #--------------------------------------------------------------------------------#
    #--------------------------------------------------------------------------------# 
    #define function that handles update of plotted experiment bias statistic 
    ...
    def select_all_stations(self):
        ...
        if self.read_instance.block_MPL_canvas_updates == False:
            ...
            if check_state == QtCore.Qt.Checked:
                self.selected_station_inds = copy.deepcopy(self.active_map_valid_station_inds) 
    ...
    def select_intersect_stations(self):
        ...
        if self.read_instance.block_MPL_canvas_updates == False:
            ...
            if check_state == QtCore.Qt.Unchecked:
                ...
                #if have only observations loaded into memory, select/unselect all plotted stations
                if len(self.read_instance.data_in_memory.keys()) == 1:
                    self.selected_station_inds = copy.deepcopy(self.active_map_valid_station_inds)
                    ...
                    #get intersect between active map valid station indices and valid station indices associated with each loaded experiment array
                    intersect_inds = np.sort(list(set.intersection(*map(set,intersect_lists))))
                    #select all intersecting stations
                    self.selected_station_inds = intersect_inds
    ...
    def on_click(self, event):  
        ...
        #get selected index of station on map
        selected_map_inds = np.array([event.ind[0]], dtype=np.int)

        #get selected station indices with respect to all available stations
        self.selected_station_inds = self.map_selected_station_inds_to_all_available_inds(selected_map_inds)
    ...
    def onlassoselect(self, verts):    
        ...
        #get selected indices of stations on map
        path = Path(verts)
        xys = self.map_points.get_offsets()
        selected_map_inds = np.nonzero(path.contains_points(xys))[0]
...
temporal_axis_mapping_dict = {'days_of_week':{0:'M', 1:'T', 2:'W', 3:'T', 4:'F', 5:'S', 6:'S'},
                              'months':      {1:'J', 2:'F', 3:'M', 4:'A', 5:'M', 6:'J', 7:'J', 8:'A', 9:'S', 10:'O', 11:'N', 12:'D'}}
