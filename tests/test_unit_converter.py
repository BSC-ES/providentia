...
possibilities = [
    ...
    (
        "kg m-1 s-2 kg-1 m s2",
        84.0,
        "g mm-1 min-2 g-1 mm min2",
        84.0,
        "relative_humidity",
        "relative_humidity",
    ),
...
@pytest.mark.parametrize(
    "orig_unit, orig_val, new_unit, new_val, input_quantity, output_quantity",
    possibilities,
)
def test_same_quantity_unit_conversion(
    orig_unit, orig_val, new_unit, new_val, input_quantity, output_quantity
):
    """Same quantity unit conversion."""

    conv_obj = unit_converter.UnitConverter(
        orig_unit,
        new_unit,
        orig_val,
...
@pytest.mark.parametrize(
    "orig_unit, orig_val, new_unit, new_val, species, valence", possibilities
)
def test_valence_species_unit_conversion(
    orig_unit, orig_val, new_unit, new_val, species, valence
):
    """Valence and species dependent unit conversions."""

    conv_obj = unit_converter.UnitConverter(
        orig_unit, new_unit, orig_val, species=species, valence=valence
    )
...
possibilities = [
    (
        ...
        "",
        {
            ...
            "pressure": "hPa",
            "molar_mass": "g mol-1",
            "mass_concentration": "ug/m3",
        },
        {
            "temperature": 293.15,
            "pressure": 1013.25,
            "molar_mass": 47.998,
            "mass_concentration": 1.9957,
        },
        "mass_concentration",
        "mole_fraction",
    ),
...
@pytest.mark.parametrize(
    "orig_unit, orig_val, new_unit, new_val, species, input_units, input_values, input_quantity, output_quantity",
    possibilities,
)
def test_different_quantity_unit_conversion(
    orig_unit,
    orig_val,
    new_unit,
    new_val,
