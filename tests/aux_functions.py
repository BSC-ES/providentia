...
def plot(inst, statistic_mode, network_type, plot_type, plot_options=[], load=True):
    ...
    fig = inst.plot(
        ...
        save_data=True,
        save_data_path=path,
        tests_generate_output=GENERATE_OUTPUT,
    )
...
def check_filter_data(inst, statistic_mode, network_type, filter):
    # Check filtered data
    filter_path = (
        f"tests/reference/{network_type}/{statistic_mode}/data/data_{filter}.npy"
    )
...
def check_unit_conversion(conv_obj, new_val, orig_val, orig_unit, new_unit, atol=1e-08):
    if not np.isclose(conv_obj.converted_value, new_val, atol=atol):
        ...
        print(
            "{} {} to {} {} correctly converted".format(
                orig_val, orig_unit, new_val, new_unit
            )
        )
...
def check_report(network_type):
    ...
    expected_report_info = extract_pdf_info(
        f"tests/reference/{network_type}/tests_{network_type}.pdf"
    )
