...
def find_model(download_instance, mod_id, domain, initial_check):
    ...
    if model_paths:
        ...
        for mod_dir in model_paths:
            ...
            if "/esarchive/" == mod_dir[:11] and download_instance.remote_hostname.startswith('transfer'):
    ...
    if model_exists is False:
        ...
        try:
            remote_dir = join(download_instance.mod_to_interp_remote_path, mod_id, domain)
            download_instance.sftp.stat(remote_dir)
            model_exists = True
        except FileNotFoundError:
            pass 
...
def find_available_resolutions(download_instance, remote_dir, mod_id, domain, initial_check):
    ...
    for resolution in download_instance.model_resolution:
        ...
        try:
            ...
            msg = (f"There is no data available in {download_instance.remote_machine} for the {mod_id} model with the "
            ...
            if not resolution_not_found:
                ...
                if sorted_resolutions:
                    while True:
                        ...
                        f"If no resolution is specified, the first available option ({sorted_resolutions[0]}) will be selected automatically. ").lower()
                    ...
                    msg = (f"There is no data available in {download_instance.remote_machine} for the {mod_id} model " 
...
def find_available_species(download_instance, valid_resolutions, remote_dir, ensemble, mod_id, domain, initial_check):
...
def find_available_nc_files(download_instance, valid_resolutions_species_dir, initial_check):
    ...
    for resolution_species_dir, mod_id, domain, resolution, species, ensemble in valid_resolutions_species_dir:
        ...
        if nc_files:
            ...
            if ensemble.isdigit() or ensemble == 'allmembers':
                ...
                local_dir = join(download_instance.mod_to_interp_root, mod_id, domain, resolution, species)
            ...
            # ensemble statistic
                ...
                local_dir = join(download_instance.mod_to_interp_root, mod_id, domain, resolution, "ensemble-stats", species + "_" + ensemble)
...
def build_nc_file_paths_in_range(download_instance, path_files_dict, initial_check):
    ...
    for dir, model_dict in path_files_dict.items():
        ...
        if not valid_nc_files:                 
            msg = (f"There is no data available in {download_instance.remote_machine} from {download_instance.start_date} to {download_instance.end_date} "
...
def download_non_interpolated_sftp(download_instance, files_to_download_info):
    for local_dir, files_to_download_dict in files_to_download_info.items():
        ...
        for nc_file in tqdm(files_to_download_dict['nc_files'], 
            ...
            download_instance.sftp.get(remote_path, local_path, callback=download_instance.check_time) 
