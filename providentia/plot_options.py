    ...
    for data_label in cut_data_labels:
        if data_label != read_instance.observations_data_label:
            ...
            regression_line = relevant_axis.plot(
                observations_data,
                m * observations_data + b,
                color=read_instance.plotting_params[data_label]["colour"],
                zorder=read_instance.plotting_params[data_label]["zorder"]
                + len(cut_data_labels),
            ...
            if read_instance.mode not in ["report"]:
                canvas_instance.plotting.track_plot_elements(
                    data_label,
                    base_plot_type,
                    "regression",
                    regression_line,
...
def smooth(
    ...
    # normal timeseries
        ...
        timeseries_data = canvas_instance.selected_station_data[networkspeci][
            "timeseries"
        ]
    ...
    for data_label in cut_data_labels:
        ...
        if "bias" in plot_options:
            ...
            # normal bias timeseries
                ...
                ts_obs = canvas_instance.selected_station_data[networkspeci][
                    "timeseries"
                ][read_instance.observations_data_label]
                ts_model = canvas_instance.selected_station_data[networkspeci][
                    "timeseries"
                ][data_label]
                ts = ts_model - ts_obs
        ...
        min_points_percentage = (
            plot_characteristics["smooth"]["min_points_percentage"] / 100
        )
        min_periods = int(
            round(
                plot_characteristics["smooth"]["window"] * (min_points_percentage / 2)
            )
        )
        smooth_line_data = ts.rolling(
            plot_characteristics["smooth"]["window"],
            min_periods=min_periods,
            center=True,
            closed="both",
        ).mean()

        smooth_line = relevant_axis.plot(
            smooth_line_data,
            color=read_instance.plotting_params[data_label]["colour"],
            zorder=read_instance.plotting_params[data_label]["zorder"]
            + len(cut_data_labels),
        ...
        if read_instance.mode not in ["report"]:
            canvas_instance.plotting.track_plot_elements(
                data_label, base_plot_type, "smooth", smooth_line, bias=bias
            )
...
def threshold(
    ...
    if base_plot_type in [
        ...
        threshold_line = relevant_axis.axhline(
            y=threshold, **plot_characteristics["threshold_line"]
        )
    ...
    if read_instance.mode not in ["report"]:
        canvas_instance.plotting.track_plot_elements(
            "ALL", base_plot_type, "threshold", [threshold_line], bias=False
        )
...
def annotation(
    ...
    plot_characteristics,
    plot_options,
    plot_z_statistic_sign="absolute",
):
    """
    ...
    if base_plot_type == "fairmode-target":
        ...
        if len(stats) == 0:
            msg = "No annotation statistics are defined for {} in plot_characteristics.yaml.".format(
                base_plot_type
            )
        ...
        if plot_characteristics["annotate_text"]["n_stations"]:
            ...
            str_to_annotate.append(
                "Stations: " + str(len(canvas_instance.station_inds[networkspeci]))
            )
        ...
        for data_label_ii, data_label in enumerate(cut_data_labels):
            ...
            for zstat in stats:
                ...
                if (
                    (bias)
                    or (plot_z_statistic_sign == "bias")
                    or (z_statistic_sign == "bias")
                ):
                    if data_label != read_instance.observations_data_label:
                        stat_calc = calculate_statistic(
                            read_instance,
                            canvas_instance,
                            networkspeci,
                            zstat,
            ...
            if plot_characteristics["annotate_text"]["color"] != "":
                colours = [plot_characteristics["annotate_text"]["color"]] * len(
                    cut_data_labels
                )
    ...
    if len(str_to_annotate) != 0:
        ...
        lines = [
            TextArea(
                line,
                textprops=dict(
                    color=colour, size=plot_characteristics["annotate_text"]["fontsize"]
                ),
            )
        ...
        if read_instance.mode not in ["report"]:
            canvas_instance.plotting.track_plot_elements(
                "ALL", base_plot_type, "annotate", [bbox], bias=bias
            )
    else:
...
def model_domain(canvas_instance, relevant_axis, data_labels, map_extent):
    ...
    grid_edge_polygons = canvas_instance.plotting.make_model_domain_polygons(
