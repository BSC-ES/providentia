...
#module which defines class required for interpolation of model data to surface observations
...
class ExperimentInterpolation(object):
    ...
    def __init__(self,submit_args):
        ...
        self.experiment_to_process                = submit_args['experiment_to_process']
        self.grid_type_to_process                 = submit_args['grid_type_to_process']
        self.model_temporal_resolution_to_process = submit_args['model_temporal_resolution_to_process']
        self.speci_to_process                     = submit_args['speci_to_process']
        self.GHOST_network_to_interpolate_against = submit_args['GHOST_network_to_interpolate_against']
        self.temporal_resolution_to_output        = submit_args['temporal_resolution_to_output']
        self.ensemble_member                      = submit_args['ensemble_member']
        self.yearmonth                            = submit_args['yearmonth']

        #create output logfile directory
        self.output_logfile_dir = '{}/interpolation_logs/{}/{}/{}/{}/{}/{}/{}/{}'.format(working_directory, self.experiment_to_process, self.grid_type_to_process, self.model_temporal_resolution_to_process, self.speci_to_process, self.GHOST_network_to_interpolate_against, self.temporal_resolution_to_output, self.ensemble_member, self.yearmonth)

        #get experiment specific directory (take gpfs experiment directory preferentially over esarchive directory)
        exp_dict = defined_experiments_dictionary[self.experiment_to_process]
        ...
        self.obs_file = glob.glob('/gpfs/projects/bsc32/AC_cache/obs/ghost/{}/{}/{}/{}/{}_{}*.nc'.format(self.GHOST_network_to_interpolate_against, GHOST_version, self.temporal_resolution_to_output, self.speci_to_process, self.speci_to_process, self.yearmonth))[0]

        #get observational file netCDF root
        self.obs_nc_root = Dataset(self.obs_file)

        #get relevant model files
        all_model_files = np.sort(glob.glob('{}/{}/{}/{}/{}*{}*.nc'.format(exp_dir, self.grid_type_to_process, self.model_temporal_resolution_to_process, self.speci_to_process, self.speci_to_process,self.yearmonth)))
        #isolate model files to be only those associated with relevant ensemble member number
        self.model_files = np.sort([f for f in all_model_files if '{}-{}_'.format(self.speci_to_process,self.ensemble_member) in f])
        #if the number of remaining model files is 0, this is because the files do not contain an ensemble member number, therefore take all model files
    ...
    def create_grid_domain_edge_polygon(self):
        ...
        if self.mod_grid_type == 'crs':
            ...
            lon_res = np.mean(np.diff(self.mod_lons_centre))
            lat_res = np.mean(np.diff(self.mod_lats_centre))
            ...
            mod_lons_edge = self.mod_lons_centre - (lon_res/2.)
            if lon_right_ind == -1:
            ...
            mod_lats_edge = self.mod_lats_centre - (lat_res/2.)
            if lat_top_ind == -1:
            ...
            self.model_grid_outline = np.vstack((lon_grid_edge,lat_grid_edge)).T
        ...
        #currently only rotated pole and lambert conformal are supported
        elif self.mod_grid_type in ['rotated_pole', 'Lambert_conformal']:
            ...
            elif self.mod_grid_type == 'Lambert_conformal':
    ...
    def get_monthly_model_data(self):
        ...
        if (self.temporal_resolution_to_output == 'hourly') or (self.temporal_resolution_to_output == 'hourly_instantaneous'):
        ...
        for model_ii, model_file in enumerate(self.model_files):
            ...
            try:
                ...
                day = int(model_file.split('_{}'.format(self.yearmonth))[-1][:2])
                ...
                adjusted_daily_file_time = diff_hours_start_day + self.mod_nc_root['time'][:]
                ...
                if len(self.mod_nc_root[self.speci_to_process].shape) == 3:
                    daily_chunked_data = self.mod_nc_root[self.speci_to_process][valid_daily_file_time_indices,:,:]
                #has vertical dimension 
                elif len(self.mod_nc_root[self.speci_to_process].shape) == 4:
                    daily_chunked_data = self.mod_nc_root[self.speci_to_process][valid_daily_file_time_indices,self.z_index,:,:] 
                ...
                if (self.temporal_resolution_to_output == 'hourly') or (self.temporal_resolution_to_output == 'hourly_instantaneous') or (self.temporal_resolution_to_output == 'monthly'): 
                    adjusted_daily_file_time = adjusted_daily_file_time[valid_daily_file_time_indices]
                    monthly_time_indices = np.arange(len(self.yearmonth_time))[np.in1d(self.yearmonth_time,adjusted_daily_file_time)]
                    self.monthly_model_data[monthly_time_indices,:,:] = daily_chunked_data

                elif self.temporal_resolution_to_output == 'daily':
                    day_ind = day-1
                    self.monthly_model_data[day_ind,:,:] = np.nanmean(daily_chunked_data, axis=0)
        ...
        if self.temporal_resolution_to_output == 'monthly':
            self.yearmonth_time = np.arange(1)
            self.monthly_model_data = np.nanmean(self.monthly_model_data, axis=0).reshape((1, self.y_N, self.x_N))
    ...
    def n_nearest_neighbour_inverse_distance_weights(self):
        ...
        #get observational lons/lats
        obs_lons = self.obs_nc_root['longitude'][:]
        obs_lats = self.obs_nc_root['latitude'][:]

        #for each pair of observational station geographic coordinates, test if station is inside model grid
        obs_inside_model_grid = np.array([self.model_grid_outline_poly.contains(Point(obs_lon,obs_lat)) for obs_lon,obs_lat in zip(obs_lons,obs_lats)]) 
        ...
        dists,idx = tree.query(obs_lonlat,k=int(n_neighbours_to_find))
    ...
    def write_yearmonth_netCDF(self):
        ...
        obs_lon_obj = self.obs_nc_root['longitude']
        obs_lat_obj = self.obs_nc_root['latitude']
        obs_station_reference_obj = self.obs_nc_root['station_reference']
        obs_measured_var_obj = self.obs_nc_root[self.speci_to_process]
        ...
        if (obs_speci_units == 'K') & (self.mod_speci_units == 'K'):
        ...
        output_dir = '/gpfs/projects/bsc32/AC_cache/recon/exp_interp/{}/{}/{}/{}/{}/{}/{}'.format(GHOST_version,self.experiment_to_process,self.grid_type_to_process,self.ensemble_member,self.temporal_resolution_to_output,self.speci_to_process,self.GHOST_network_to_interpolate_against)
        ...
        try:
            ...
            pass
        netCDF_fname = '{}/{}_{}.nc'.format(output_dir,self.speci_to_process,self.yearmonth)
        ...
        #file contents
        root_grp.title = 'Inverse distance weighting ({} neighbours) interpolated {} experiment data for the component {} with reference to the measurement stations in the {} network in {}-{}.'.format(n_neighbours_to_find, self.experiment_to_process, self.speci_to_process, self.GHOST_network_to_interpolate_against, self.year, self.month)
        ...
        for ii, station_reference in enumerate(obs_station_reference_obj[:]):
            ...
            if ii == 0:
                ...
                time_var.units = '{} since {}-{}-01 00:00:00'.format(descriptive_temporal_resolution,self.year,self.month)
                time_var.description = 'Time in {} since {}-{}-01 00:00 UTC. Time given refers to the start of the time window the measurement is representative of (temporal resolution).'.format(descriptive_temporal_resolution,self.year,self.month)
                time_var.axis = 'T'
                ...
                model_centre_longitude_var.description = '2D meshed grid centre longitudes with {} longitudes in {} bands of latitude'.format(self.x_N,self.y_N)
                ...
                model_centre_latitude_var.description = '2D meshed grid centre latitudes with {} latitudes in {} bands of longitude'.format(self.y_N,self.x_N)
                ...
                measured_var = root_grp.createVariable(self.speci_to_process, 'f4', ('station','time'))
                ...
                measured_var.descrption = 'Interpolated value of {} from the experiment {} with reference to the measurement stations in the {} network'.format(obs_measured_var_obj.standard_name, self.experiment_to_process, self.GHOST_network_to_interpolate_against)
            ...
            if np.all(station_weights == 0):
                ...
                cut_model_data = self.monthly_model_data[:,self.nearest_neighbour_inds[ii,:n_neighbours_to_find],self.nearest_neighbour_inds[ii,n_neighbours_to_find:]]
...
if __name__ == "__main__":
    try:
        ...
        submit_args = {'experiment_to_process':sys.argv[1], 'grid_type_to_process':sys.argv[2], 'model_temporal_resolution_to_process':sys.argv[3], 'speci_to_process':sys.argv[4], 'GHOST_network_to_interpolate_against':sys.argv[5], 'temporal_resolution_to_output':sys.argv[6], 'ensemble_member':sys.argv[7], 'yearmonth':sys.argv[8]}
