...
class convert_units:
    ''' Class that converts units.
        ...
        and converts between different quantities (i.e. mass density --> mole fraction, mg/m3 --> ppbv) **when given 
        the correct parameters needed/ and have formula

        **units which are solely per something e.g. frequency (s-1) are prefixed with 'xx' to prevent confusion between 
        units
        i.e. ms-1 (per miliseconds) could be taken to be m s-1 (metres per second) or vice versa in processing  

        requires input of: 
        1. input units -- either single string for same quantity conversion, or a dictionary of input quantity name and 
        units for converting between quantities {'quantity':unit1, 'quantity2':unit2}  
        **quantity names should be given with underscore joins when more 1 word 

        2. output units

        3. input value/s -- either a single value for same quantity conversion, or a dictionary of input quantity name 
        and values for converting between quantities {'quantity':value, 'quantity2':value2} 
        **quantity names should be given with underscore joins when more 1 word 

        4. Level of precision of converted output/conversion factor (fixed number of decimal places) **OPTIONAL 

        5. Measured species (in chemical notation). **OPTIONAL -- needed when converting units associated with an 
        element (e.g. ppmv N) 

        6. Conversion input quantity **OPTIONAL -- used when wanting to return conversion factor for conversions with 
        more than 1 input variables, i.e. this quantity refers to the input variable for which a conversion factor 
        to the output units is wanted
        ...
        conv_obj = unit_converter.convert_units(input_units, output_units, input_values, 
                                                conversion_input_quantity='mass_density')
        conv_obj.do_conversion()
    ...
    def do_conversion(self):
        ...
        # if input unit is just a string, get conversion factor to convert between given input and output units to 
        # standard SI
        # the ratio between these conversion factors * the input value then gives the desired output value in the 
        # desired units   
        ...
        # process each of the input units into standard SI units, and then calculate output value using conversion formula
            ...
            if self.output_standard_units != self.output_reference_units:
                ...
                if ((self.output_represented_quantity == 'temperature') 
    ...
    def check_quantity_agreement(self):
        ...
        if self.input_represented_quantity != self.output_represented_quantity:
            sys.exit('Input and Output Quantities do not agree: {}, {}'.format(self.input_represented_quantity, 
                                                                               self.output_represented_quantity))
    ...
    def get_conversion_formula(self, formula_type='standard'):
        ...
        # is the conversion formula a factor (i.e. formula to get factor between an input and output quantity)
            ...
            # if don't have formula, return error message
            if ('{}>{}--conversion_factor'.format(self.conversion_input_quantity, self.output_represented_quantity) 
    ...
    def standardise_unit_form(self, units, unit_type):
        ...
        match_list, reference_units, standard_units, scaling_factors_to_reference = map(list, zip(*sorted(zip(match_list, 
            reference_units, standard_units, scaling_factors_to_reference), key=lambda item: (-len(item[0]), item[0]))))
    ...
    def load_dictionaries(self):
        ...
        self.SI_base_quantities = {
                                   ...
                                   # blank unit only used to prevent confusion between units for rates 
                                   # (i.e. frequency - s-1)
                                   'blank'              :{'unit_name':'blank',
                                                          'base_units':'xx',
                                                          'equiv_units':['none']}}                     
        ...
        self.SI_derived_quantities = {
                                      ...
                                      'angle':                        {'unit_name':'radian',
                                                                       'base_units':'m m-1',
                                                                       'standard_units':'rad',
                                                                       'equiv_units':['rad','rads','radian','radians']},
                                      'area':                         {'unit_name':'square metre', 
                                                                       'base_units':'m2',
                                                                       'standard_units':'m2',
                                                                       'equiv_units':['none']},
                                      'area_density':                 {'unit_name':'kilogram per square metre',
                                                                       'base_units':'m-2 kg',
                                                                       'standard_units':'m-2 kg',
                                                                       'equiv_units':['none']},
                                      ...
                                      'magnetic_field_strength':      {'unit_name':'tesla',
                                                                       ...
                                                                       'standard_units':'T', 
                                                                       'equiv_units':['T','tesla']},
                                      'mass_density':                 {'unit_name':'kilogram per cubic metre',
                                                                       'base_units':'m-3 kg', 
                                      ...
                                      'solid_angle':                  {'unit_name':'steradian', 
                                                                       'base_units':'m2 m-2',
                                                                       'standard_units':'sr',
                                                                       'equiv_units':['sr','steradian','steradians']},
                                      ...
                                      'spectral_irradiance':          {'unit_name':'watt per cubic metre', 
                                                                       'base_units':'m-1 kg s-3', 
                                                                       'standard_units':'m-1 kg s-3', 
                                                                       'equiv_units':['none']},
                                      'temperature_gradient':         {'unit_name':'kelvin per metre', 
                                                                       'base_units':'m-1 K', 
                                                                       'standard_units':'m-1 K', 
                                                                       'equiv_units':['none']},
                                      'thermal_resistance':           {'unit_name':'kelvin per watt', 
                                                                       'base_units':'m-2 kg-1 s3 K',
                                                                       'standard_units':'m-2 kg-1 s3 K', 
                                                                       'equiv_units':['none']},
