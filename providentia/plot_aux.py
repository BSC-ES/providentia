...
def compute_fairmode_MQI(o, m, u_95r_RV, RV, alpha):
    """ Function from pyeslib with minor modifications
    """ 
    ...
    if len(wok) != 0:
        ...
        rmse = np.sqrt(np.nanmean((o - m) ** 2))
        mqi = rmse / (beta * rms_u)
        return (mqi)
    else:
        return (np.nan)

def plot_fairmode(o, m, u_95r_RV, RV, alpha, percentile):
    """ Function from pyeslib with minor modifications
    """ 
    ...
    if len(wok) != 0:
        ...
        rmsu = u_95r_RV * np.sqrt((1 - alpha ** 2) * (np.nanmean(o) ** 2 + np.nanstd(o) ** 2) + (alpha * RV) ** 2)

        m_perc = np.percentile(m, percentile)
        o_perc = np.percentile(o, percentile)

        t_bias = bias / (beta * rmsu)
        t_R = (1 - scipy.stats.pearsonr(o, m)[0]) / ((0.5 * (beta ** 2) * rmsu * rmsu) / (np.nanstd(o) * np.nanstd(m)))
        t_sd = ((np.nanstd(m) - np.nanstd(o))) / (beta * rmsu)
        hiper = (m_perc - o_perc) / (
                    beta * u_95r_RV * np.sqrt((1 - alpha ** 2) * np.nanmean(o) ** 2 + (alpha * RV) ** 2))

        x = np.sqrt(
            np.mean(((np.array(m) - np.mean(m)) - (np.array(o) - np.mean(o))) ** 2)
        ) / (beta * u_95r_RV * np.sqrt((1 - alpha ** 2) * (np.nanmean(o) ** 2 + np.nanstd(o) ** 2) + (alpha * RV) ** 2))
        
        # CRMSE/BETA*RMSu
        y = bias / (beta * rmsu)
        
        # BIAS/BETA*RMSu
        ratio = np.abs((np.nanstd(m) - np.nanstd(o))) / (
                    np.nanstd(o) * np.sqrt(2 * (1 - scipy.stats.pearsonr(o, m)[0])))
