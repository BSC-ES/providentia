...
class DataReader:
    ...
    def read_setup(self, operations, experiments_to_remove=None, experiments_to_read=None):
        ...
        if 'reset' in operations:  
            ...
            invalid_args = {field_name:fields-set(self.read_instance.metadata_vars_to_read) for field_name,fields in self.read_instance.non_default_fields_per_section.items()}
            invalid_var = [f"""{i} ('{"', '".join(j)}')""" for i,j in invalid_args.items() if j]
            if invalid_var:
                msg = f"Invalid field(s) in configuration file {self.read_instance.config.split('/')[-1]}. Section(s) and Field(s): {', '.join(invalid_var)}."
                show_message(self.read_instance, msg)
