...
class SpatialColocation:
    ...
    def __init__(self, read_instance):
        ...
        self.intersecting_indices = {
            networkspeci: [] for networkspeci in self.networkspecies
        }
        ...
        if (self.read_instance.spatial_colocation_measurement_altitude) & (
            not self.read_instance.spatial_colocation_longitude_latitude
        ):
        ...
        # if not wanting to use any variables for spatial colcoation, then return as there is nothing to be done
            ...
            (not self.read_instance.spatial_colocation_station_reference)
            & (not self.read_instance.spatial_colocation_station_name)
            & (not self.read_instance.spatial_colocation_longitude_latitude)
            & (not self.read_instance.spatial_colocation_measurement_altitude)
        ):
    ...
    def by_station_reference(self):
        ...
        if self.read_instance.reading_ghost:
            station_references_cut = {
                k: np.array(["_".join(ref.split("_")[:-1]) for ref in v])
                for k, v in self.read_instance.station_references.items()
            }
        else:
            station_references_cut = copy.deepcopy(
                self.read_instance.station_references
            )

        # get intersecting station references
        intersecting_station_references = list(
            set.intersection(*map(set, list(station_references_cut.values())))
        )
        ...
        if len(intersecting_station_references) > 0:
            ...
            for networkspeci in station_references_cut:
                unique_refs, unique_counts = np.unique(
                    station_references_cut[networkspeci], return_counts=True
                )
            ...
            intersecting_station_references = np.array(
                [
                    ref
                    for ref in intersecting_station_references
            ...
            for networkspeci in station_references_cut:
                station_references_sorted = np.argsort(
                    station_references_cut[networkspeci]
                )
                intersect_refs_pos = np.searchsorted(
                    station_references_cut[networkspeci][station_references_sorted],
                    intersecting_station_references,
                )
                self.intersecting_indices[networkspeci] = np.array(
                    np.append(
                        self.intersecting_indices[networkspeci],
    ...
    def by_station_name(self):
        ...
        if len(self.intersecting_indices[self.firstnetworkspeci]) != len(
            self.read_instance.station_references[self.firstnetworkspeci]
        ):
            # get intersecting station names
            intersecting_station_names = list(
                set.intersection(
                    *map(set, list(self.non_intersecting_station_names.values()))
                )
            )
            ...
            if len(intersecting_station_names) > 0:
                ...
                for networkspeci in self.read_instance.station_names:
                    unique_names, unique_counts = np.unique(
                ...
                intersecting_station_names = np.array(
                    [
                        name
                        for name in intersecting_station_names
                ...
                for networkspeci in self.read_instance.station_names:
                    station_names_sorted = np.argsort(
                        self.read_instance.station_names[networkspeci]
                    )
                    intersect_names_pos = np.searchsorted(
                        self.read_instance.station_names[networkspeci][
                            station_names_sorted
                        ],
                        intersecting_station_names,
                    )
                    self.intersecting_indices[networkspeci] = np.array(
                        np.append(
                            self.intersecting_indices[networkspeci],
    ...
    def by_position(self):
        ...
        if len(self.intersecting_indices[self.firstnetworkspeci]) != len(
            ...
            firstnetworkspeci_longitudes = self.non_intersecting_longitudes[
                self.firstnetworkspeci
            ]
            firstnetworkspeci_latitudes = self.non_intersecting_latitudes[
                self.firstnetworkspeci
            ]
            if self.read_instance.spatial_colocation_measurement_altitude:
                firstnetworkspeci_measurement_altitudes = (
                    self.non_intersecting_measurement_altitudes[self.firstnetworkspeci]
                )
            else:
                firstnetworkspeci_measurement_altitudes = np.zeros(
                    len(firstnetworkspeci_longitudes)
                )
            ...
            if Version(pyproj.__version__) >= Version("2.0.0"):
                (
                    ...
                    firstnetworkspeci_longitudes,
                    firstnetworkspeci_latitudes,
                    firstnetworkspeci_measurement_altitudes,
                    radians=False,
                )
            else:
            ...
            self.firstnetworkspeci_xyz = np.column_stack(
                (firstnetworkspeci_x, firstnetworkspeci_y, firstnetworkspeci_z)
            )
            ...
            for networkspeci in self.non_intersecting_longitudes:
                ...
                nextnetworkspeci_longitudes = self.non_intersecting_longitudes[
                    networkspeci
                ]
                nextnetworkspeci_latitudes = self.non_intersecting_latitudes[
                    networkspeci
                ]
                if self.read_instance.spatial_colocation_measurement_altitude:
                    nextnetworkspeci_measurement_altitudes = (
                        self.non_intersecting_measurement_altitudes[networkspeci]
                    )
                else:
                    nextnetworkspeci_measurement_altitudes = np.zeros(
                        len(nextnetworkspeci_longitudes)
                    )
                ...
                if Version(pyproj.__version__) >= Version("2.0.0"):
                    (
                        ...
                        nextnetworkspeci_longitudes,
                        nextnetworkspeci_latitudes,
                        nextnetworkspeci_measurement_altitudes,
                        radians=False,
                    )
                else:
                ...
                # merge coordinates to 3D array
                self.nextnetworkspeci_xyz = np.column_stack(
                    (nextnetworkspeci_x, nextnetworkspeci_y, nextnetworkspeci_z)
                )
                ...
                if Version(scipy.__version__) < Version("1.6.0"):
                    self.idx = cKDTree(self.nextnetworkspeci_xyz).query_ball_point(
                        self.firstnetworkspeci_xyz,
                        self.read_instance.spatial_colocation_tolerance,
                    )
                else:
                    self.idx = cKDTree(self.nextnetworkspeci_xyz).query_ball_point(
                        self.firstnetworkspeci_xyz,
                ...
                if len(self.fs_wtol_inds) > 0:
                    ...
                    pairwise_intersect_inds[
                        "{}_{}".format(self.firstnetworkspeci, networkspeci)
                    ] = self.non_intersecting_indices[self.firstnetworkspeci][
                        np.array(self.fs_wtol_inds)
                    ]
                    pairwise_intersect_inds[self.firstnetworkspeci].extend(
                        self.non_intersecting_indices[self.firstnetworkspeci][
                            np.array(self.fs_wtol_inds)
                        ]
                    )
                    ...
                    pairwise_intersect_inds[
                        networkspeci
                    ] = self.non_intersecting_indices[networkspeci][
                        np.array(self.ns_wtol_inds)
                    ]
            ...
            pairwise_intersect_inds_unique, counts = np.unique(
                pairwise_intersect_inds[self.firstnetworkspeci], return_counts=True
            )
            pairwise_intersect_inds[
                self.firstnetworkspeci
            ] = pairwise_intersect_inds_unique[
                counts == (len(self.read_instance.station_longitudes) - 1)
            ]
            ...
            if len(pairwise_intersect_inds[self.firstnetworkspeci]) > 0:
                for networkspeci in self.non_intersecting_longitudes:
                    if networkspeci == self.firstnetworkspeci:
                        continue
                    _, species_intersect_inds, _ = np.intersect1d(
                        pairwise_intersect_inds[
                            "{}_{}".format(self.firstnetworkspeci, networkspeci)
                        ],
                        pairwise_intersect_inds[self.firstnetworkspeci],
                    ...
                    pairwise_intersect_inds[networkspeci] = pairwise_intersect_inds[
                        networkspeci
                    ][species_intersect_inds]
                ...
                for networkspeci in self.non_intersecting_longitudes:
                    self.intersecting_indices[networkspeci] = np.array(
                        np.append(
                            self.intersecting_indices[networkspeci],
                            pairwise_intersect_inds[networkspeci],
                        ),
                        dtype=np.int32,
                    )
        ...
        if self.read_instance.spatial_colocation_validation:
            ...
            for stn_ii in range(len(self.intersecting_indices[self.firstnetworkspeci])):
                ...
                for networkspeci in self.networkspecies:
                    station_lons = np.append(
                        station_lons,
                        self.read_instance.station_longitudes[networkspeci][
                            self.intersecting_indices[networkspeci][stn_ii]
                        ],
                    )
                    station_lats = np.append(
                        station_lats,
                ...
                lon_diff_max = np.max(
                    np.abs(station_lons_ecef - station_lons_ecef[:, np.newaxis])
                )
                lat_diff_max = np.max(
                    np.abs(station_lats_ecef - station_lats_ecef[:, np.newaxis])
                )
                if (
                    lon_diff_max
                    > self.read_instance.spatial_colocation_validation_tolerance
                ) or (
                    lat_diff_max
                    > self.read_instance.spatial_colocation_validation_tolerance
                ):
            ...
            for networkspeci in self.networkspecies:
                self.intersecting_indices[networkspeci] = np.array(
                    [
                        inter_ii
                        for stn_ii, inter_ii in enumerate(
                            self.intersecting_indices[networkspeci]
                        )
                        if stn_ii not in stn_inds_to_drop
                    ],
                    dtype=np.int32,
                )
    ...
    def sort_intersecting_indices(self):
        ...
        for networkspeci in self.networkspecies:
            self.intersecting_indices[networkspeci] = self.intersecting_indices[
                networkspeci
            ][sorted_inds]
    ...
    def get_non_intersections(self):
        ...
        self.non_intersecting_indices = {
            networkspeci: np.setdiff1d(
                np.arange(len(self.read_instance.station_references[networkspeci])),
                self.intersecting_indices[networkspeci],
            )
            for networkspeci in self.networkspecies
        }
        ...
        if self.read_instance.spatial_colocation_station_name:
            self.non_intersecting_station_names = {
                networkspeci: self.read_instance.station_names[networkspeci][
                    self.non_intersecting_indices[networkspeci]
                ]
                for networkspeci in self.read_instance.station_names
            }
        if self.read_instance.spatial_colocation_longitude_latitude:
            self.non_intersecting_longitudes = {
                networkspeci: self.read_instance.station_longitudes[networkspeci][
                    self.non_intersecting_indices[networkspeci]
                ]
                for networkspeci in self.read_instance.station_longitudes
            }
            self.non_intersecting_latitudes = {
                networkspeci: self.read_instance.station_latitudes[networkspeci][
                    self.non_intersecting_indices[networkspeci]
                ]
                for networkspeci in self.read_instance.station_latitudes
            }
        if self.read_instance.spatial_colocation_measurement_altitude:
            self.non_intersecting_measurement_altitudes = {
                networkspeci: self.read_instance.station_measurement_altitudes[
                    networkspeci
                ][self.non_intersecting_indices[networkspeci]]
                for networkspeci in self.read_instance.station_measurement_altitudes
            }
    ...
    def resolve_duplicate_spatial_colocation_matches(self):
        ...
        unique_idx, unique_idx_counts = np.unique(
            list(itertools.chain(*self.idx)), return_counts=True
        )
        ...
        while True:
            ...
            for idx_ii, idx_l in enumerate(self.idx):
                if len(idx_l) > 1:
                    ...
                    if Version(scipy.__version__) < Version("1.6.0"):
                        dists = cKDTree([self.firstnetworkspeci_xyz[idx_ii]]).query(
                            self.nextnetworkspeci_xyz[idx_l], k=1
                        )[0]
                    else:
                        dists = cKDTree([self.firstnetworkspeci_xyz[idx_ii]]).query(
                            self.nextnetworkspeci_xyz[idx_l], k=1, workers=-1
                        )[0]
            ...
            for dup_index in dup_idx:
                ...
                relevant_idx = np.array(
                    [
                        idx_ii
                        for idx_ii, idx_l in enumerate(self.idx)
                        if dup_index in idx_l
                    ]
                )
                ...
                if Version(scipy.__version__) < Version("1.6.0"):
                    dists = cKDTree([self.nextnetworkspeci_xyz[dup_index]]).query(
                        self.firstnetworkspeci_xyz[relevant_idx], k=1
                    )[0]
                else:
                    dists = cKDTree([self.nextnetworkspeci_xyz[dup_index]]).query(
                        self.firstnetworkspeci_xyz[relevant_idx], k=1, workers=-1
                    )[0]
                ...
                for ordered_relevant_index_ii, ordered_relevant_index in enumerate(
                    ordered_relevant_idx
                ):
                    ...
                    if len(self.idx[ordered_relevant_index]) == 1:
                        ...
                        for next_ordered_relevant_index in ordered_relevant_idx[
                            ordered_relevant_index_ii + 1 :
                        ]:
                    ...
                    # otherwise, find the dists between the relevant first speci xyz coord and next speci duplicate xyz coords
                        ...
                        if Version(scipy.__version__) < Version("1.6.0"):
                            dists = cKDTree(
                                [self.firstnetworkspeci_xyz[ordered_relevant_index]]
                            ).query(self.nextnetworkspeci_xyz[idx_l], k=1)[0]
                        else:
                            dists = cKDTree(
                                [self.firstnetworkspeci_xyz[ordered_relevant_index]]
                            ).query(self.nextnetworkspeci_xyz[idx_l], k=1, workers=-1)[
                                0
                            ]
                        ordered_idx_l = idx_l[np.argsort(dists)]
                        ...
                        if (ordered_idx_l[0] == dup_index) or (
                            ordered_idx_l[0] in nondup_idx
                        ):
                            self.fs_wtol_inds.append(ordered_relevant_index)
                            self.ns_wtol_inds.append(ordered_idx_l[0])
                            if ordered_idx_l[0] == dup_index:
                                ...
                                for next_ordered_relevant_index in ordered_relevant_idx[
                                    ...
                                    self.idx[next_ordered_relevant_index].remove(
                                        dup_index
                                    )
                                break
