""" Navigation toolbar and buttons/options functions"""
import os
import configparser

from PyQt5 import QtCore, QtWidgets, QtGui
import matplotlib
from matplotlib.backends import qt_compat
from matplotlib.backends.backend_qt5agg import NavigationToolbar2QT

from .writing import export_data_npz, export_netcdf, export_configuration
from providentia import aux

CURRENT_PATH = os.path.abspath(os.path.dirname(__file__))

class NavigationToolbar(NavigationToolbar2QT):
    """Define class that updates available buttons on matplotlib toolbar"""
    
    def __init__(self, parent_instance=None, canvas_instance=None):
        
        self.parent_instance = parent_instance
        self.canvas_instance = canvas_instance
        
        # only display wanted buttons
        NavigationToolbar2QT.toolitems = (
            ('Save data', 'Save current instance of data and metadata', '', 'save_data'),
            ('Load data', 'Load toolbar selections from configuration file', '', 'conf_dialogs'),
            (None, None, None, None),
            ('Home', 'Reset original view', 'home', 'home'),
            ('Back', 'Back to previous view', 'back', 'back'),
            ('Forward', 'Forward to next view', 'forward', 'forward'),
            (None, None, None, None),
            ('Zoom', 'Zoom to rectangle', 'zoom_to_rect', 'zoom'),
            ('Pan', 'Pan axes with left mouse, zoom with right', 'move', 'pan'),
            ('Save figure', 'Save the figure', 'filesave', 'save_figure'),
        )

        super(NavigationToolbar, self).__init__(canvas_instance, parent_instance)

        actions = self.findChildren(QtWidgets.QAction)
        self._actions['save_data'].setIcon(QtGui.QIcon(os.path.join(CURRENT_PATH, "resources/save_icon.png")))
        self._actions['conf_dialogs'].setIcon(QtGui.QIcon(os.path.join(CURRENT_PATH, "resources/conf_icon.png")))
        self._actions['zoom'].setIcon(QtGui.QIcon(os.path.join(CURRENT_PATH, "resources/zoom_icon.png")))
        self._actions['save_figure'].setIcon(QtGui.QIcon(os.path.join(CURRENT_PATH, "resources/save_fig_icon.png")))
        
    def save_data(self):
        """Pops window for choosing directory, filename and type
        for saving data, metadata and configuration.
        Available filetypes: Numpy file: .npz, netCDF: .nc"""

        filetypes = {'NetCDF': 'nc', 'Numpy file': 'npz', 'Configuration': 'conf'}
        sorted_filetypes = sorted(filetypes.items())
        startpath = os.path.expanduser(matplotlib.rcParams['savefig.directory'])
        daterange = self.canvas_instance.read_instance.le_start_date.text() + "_" \
                    + self.canvas_instance.read_instance.le_end_date.text()
        default_name = "PRV_" + str(self.canvas_instance.read_instance.species[0]) + "_" + daterange
        start = os.path.join(startpath, default_name)

        filter_ext = ['%s (%s)' % (name, '*.%s' % ext) for name, ext in sorted_filetypes]
        filter_ext = ';;'.join(filter_ext)
        # prompt with file name and extension
        fname, fext = qt_compat._getSaveFileName(None, "Choose a filename to save to", start, filter_ext)
        chose_npz = "npz" in fext
        chose_conf = "conf" in fext
        if fname:
            # Save dir for next time, unless empty str (i.e., use cwd).
            if startpath != "":
                matplotlib.rcParams['savefig.directory'] = (os.path.dirname(fname))
                try:
                    QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
                    if chose_npz:
                        export_data_npz(self.canvas_instance, fname)
                    elif chose_conf:
                        export_configuration(self.canvas_instance.read_instance, fname)
                    else:
                        export_netcdf(self.canvas_instance, fname)
                    QtWidgets.QApplication.restoreOverrideCursor()
                except Exception as e:
                    QtWidgets.QMessageBox.critical(self.canvas_instance, "Error saving file", str(e),
                                                   QtWidgets.QMessageBox.Ok, QtWidgets.QMessageBox.NoButton)

    def conf_dialogs(self):
        """Pops window for selecting configuration file. If file selcted, pops an
        input dialog for the user to select which section wants to load. Calls
        reload_conf to reset the fields"""

        read_instance = self.canvas_instance.read_instance

        conf_to_load = self.filename_dialog(read_instance)
        
        # is user pressed cancel
        if conf_to_load is None:
            return

        try:
            aux.load_conf(read_instance, fpath=conf_to_load)
            all_sections = read_instance.sub_opts.keys()
            if len(all_sections) == 1:
                okpressed = False
                selected_section = list(all_sections)[0]
            else:
                selected_section, okpressed = QtWidgets.QInputDialog.getItem(read_instance, 'Sections',
                                                                             'Select section to load',  
                                                                             all_sections, 0, False)
            if okpressed or len(all_sections) == 1:
                self.reload_conf(read_instance, selected_section, conf_to_load)
        except Exception as e:
            QtWidgets.QMessageBox.critical(read_instance, "Error loading configuration file",
                                           str(e), QtWidgets.QMessageBox.Ok, QtWidgets.QMessageBox.NoButton)

    def filename_dialog(self, read_instance):
        "Open dialog to choose configuration file."

        options =  QtWidgets.QFileDialog.Options()
        options |=  QtWidgets.QFileDialog.DontUseNativeDialog
        filename, _ =  QtWidgets.QFileDialog.getOpenFileName(read_instance, "Choose configuration file to load", "",
                                                             "All Files (*);;Python Files (*.py", options=options)
        if filename:
            return filename

    def reload_conf(self, read_instance, section, fpath):
        """Resets previous selections, fills values according to new conf file,
        reads and filters."""

        # delete attributes from default config file
        CURRENT_PATH = os.path.abspath(os.path.dirname(__file__))
        dconf_path = (os.path.join(CURRENT_PATH, 'conf/default.conf'))
        dconf = configparser.RawConfigParser(empty_lines_in_values=False)
        dconf.read(dconf_path)
        if read_instance.from_conf:
            for k in dconf['default'].keys():
                delattr(read_instance, k)

        # update config and section attributes of instance
        read_instance.config = fpath
        read_instance.section = section
        read_instance.from_conf = True
        vars(read_instance).update({(k, read_instance.parse_parameter(k, val)) for k, val in read_instance.sub_opts[section].items()})

        # update species, experiments, qa & flags
        read_instance.config_bar_initialisation = True
        read_instance.update_configuration_bar_fields()
        read_instance.config_bar_initialisation = False
        
        # read
        read_instance.handle_data_selection_update()

        # reset the filter fields
        read_instance.reset_options()

        # set fields available for filtering
        aux.representativity_conf(read_instance)
        aux.period_conf(read_instance)
        aux.metadata_conf(read_instance)
        
        # filter
        read_instance.mpl_canvas.handle_data_filter_update()
