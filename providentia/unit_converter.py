...
# ---------------------------------------------------------------------------
# Static unit-system reference data.
#
# These are identical for every UnitConverter instance (no dependency on
# species/valence/units), so they are built once here at import time instead
# of being re-literalised inside load_dictionaries() on every __init__ call.
# ---------------------------------------------------------------------------
...
_SI_DERIVED_QUANTITIES = {
    ...
    "action": {
        "unit_name": "joule second",
        "base_units": "m2 kg s-1",
        "standard_units": "m2 kg s-1",
        "equiv_units": ["none"],
    },
    "angular_acceleration": {
        "unit_name": "radian per second squared",
        "base_units": "s-2",
        "standard_units": "s-2",
        "equiv_units": ["none"],
    },
    "angular_momentum": {
        "unit_name": "newton metre second",
        "base_units": "m2 kg s-1",
        "standard_units": "m2 kg s-1",
        "equiv_units": ["none"],
    },
    ...
    "column_number_density": {
        "unit_name": "number per square metre",
        "base_units": "m-2",
        "standard_units": "m-2",
        "equiv_units": ["none"],
    },
   ...
   "irradiance": {
    ...
    "luminance": {
        "unit_name": "candela per square metre",
        "base_units": "cd m-2",
    ...
    "mass_absorption_cross_section": {
        "unit_name": "area per mass",
        "base_units": "m2 kg-1",
        "standard_units": "m2 kg-1",
        "equiv_units": ["none"],
    },
    "mass_area_density": {
        "unit_name": "kilogram per square metre",
        "base_units": "kg m-2",
    ...
    "mass_flux": {
        "unit_name": "kilogram per square metre per second",
        "base_units": "kg m-2 s-1",
        "standard_units": "kg m-2 s-1",
        "equiv_units": ["none"],
    },
    "mass_fraction": {
        "unit_name": "kilogram per kilogram",
        "base_units": "kg kg-1",
        "standard_units": "kg kg-1",
        "equiv_units": ["unitless"],
    },
    "molarity": {
        "unit_name": "mole per cubic metre",
        "base_units": "mol m-3",
        "standard_units": "M",
        "equiv_units": ["Molar", "molar", "M"],
    },
    "molar_heat_capacity": {
        "unit_name": "joule per kelvin mole",
        "base_units": "m2 kg s-2 K-1 mol-1",
        "standard_units": "m2 kg s-2 K-1 mol-1",
        "equiv_units": ["none"],
    },
    "molar_mass": {
        "unit_name": "kilogram per mole",
        "base_units": "kg mol-1",
        "standard_units": "kg mol-1",
        "equiv_units": ["none"],
    },
    "molar_volume": {
        "unit_name": "cubic metre per mole",
        "base_units": "m3 mol-1",
        "standard_units": "m3 mol-1",
        "equiv_units": ["none"],
    },
    "momentum": {
        "unit_name": "newton second",
        "base_units": "m kg s-1",
        "standard_units": "m kg s-1",
        "equiv_units": ["none"],
    },
    "mole_fraction": {
        "unit_name": "mole per mole",
        "base_units": "mol mol-1",
        "standard_units": "mol mol-1",
        "equiv_units": ["unitless"],
    },
    "number_density": {
        "unit_name": "number per cubic metre",
        "base_units": "m-3",
        "standard_units": "m-3",
        "equiv_units": ["none"],
    },
    "optical_depth": {
        "unit_name": "extinction coefficient per path length",
        "base_units": "m m-1",
    ...
    "population_density": {
        "unit_name": "population per square metre",
        "base_units": "m-2",
        "standard_units": "m-2",
        "equiv_units": ["none"],
    },
    ...
    "refractive_index": {
        "unit_name": "metre per second per metre per second",
        "base_units": "m s-1 m-1 s",
        "standard_units": "m s-1 m-1 s",
        "equiv_units": ["unitless"],
    },
    "relative_humidity": {
        "unit_name": "pascal per pascal",
        "base_units": "kg m-1 s-2 kg-1 m s2",
        "standard_units": "kg m-1 s-2 kg-1 m s2",
        "equiv_units": ["unitless"],
    },
    "snap": {
        "unit_name": "metre per second to the fourth",
        "base_units": "m s-4",
    ...
    "single_scattering_albedo": {
        "unit_name": "scattering coefficient per extinction coefficient",
        "base_units": "m m-1",
        "standard_units": "m m-1",
        "equiv_units": ["unitless"],
    },
    ...
    "volume": {
        "unit_name": "cubic metre",
        "base_units": "m3",
        "standard_units": "m3",
        "equiv_units": ["none"],
    },
    "volume_concentration": {
        "unit_name": "cubic metre per square metre",
        "base_units": "m3 m-2",
        "standard_units": "m3 m-2",
        "equiv_units": ["none"],
    },
    "volume_scattering_function": {
        "unit_name": "reciprocal metre per steradian",
        "base_units": "m-1 m2 m-2",
        "standard_units": "m-1 m2 m-2",
        "equiv_units": ["none"],
    },
    "volumetric_flow": {
        "unit_name": "cubic metre per second",
        "base_units": "m3 s-1",
...
def _canonical_unit_signature(units):
    """
    Character-multiset signature of a unit string.

    Matches the semantics of the historical `Counter(units) == Counter(other)`
    comparison used throughout this module to test whether two unit strings
    represent the same quantity (order-independent, character-for-character).
    Hashable, so it can be used as a dict key.
    """
    return tuple(sorted(Counter(units).items()))


# Precomputed once at import time: quantity name -> canonical signature of its
# reference base_units, plus the reverse lookup (signature -> quantity names,
# in the same order quantities are defined in below) and the combined name
# order across all three dicts (there are no name collisions between them).
# get_quantity_representation() uses these to identify which quantity a unit
# string represents via a lookup instead of rescanning every known quantity
# (~80 entries, several Counter() comparisons each) on every call.
_QUANTITY_ORDER_NAMES = (
    list(_SI_BASE_QUANTITIES) + list(_SI_DERIVED_QUANTITIES) + list(_CONSTANTS)
)
_QUANTITY_INFO_BY_NAME = {**_SI_BASE_QUANTITIES, **_SI_DERIVED_QUANTITIES, **_CONSTANTS}
_QUANTITY_SIGNATURE_LOOKUP = {}
for _quantity_name in _QUANTITY_ORDER_NAMES:
    _signature = _canonical_unit_signature(
        _QUANTITY_INFO_BY_NAME[_quantity_name]["base_units"]
    )
    _QUANTITY_SIGNATURE_LOOKUP.setdefault(_signature, []).append(_quantity_name)
del _quantity_name, _signature


# quantity conversion formulae
_CONVERSION_FORMULAE = {
    "mole_fraction": [
        "mass_concentration*((molar_gas_constant*temperature)/(molar_mass*pressure))",
        "number_density*((molar_gas_constant*temperature)/(avogadro_constant*pressure))",
        "mass_fraction*(molar_mass_dry_air/molar_mass)",
    ],
    "mass_fraction": [
        "mole_fraction*(molar_mass/molar_mass_dry_air)",
        "mass_concentration*((molar_gas_constant*temperature)/(pressure*molar_mass_dry_air))",
        "number_density*((molar_gas_constant*temperature*molar_mass_dry_air)/(avogadro_constant*molar_mass*pressure))",
        "molarity*((molar_gas_constant*temperature*molar_mass)/(pressure*molar_mass_dry_air))",
    ],
    "mass_concentration": [
        "mole_fraction*((molar_mass*pressure)/(molar_gas_constant*temperature))",
        "(number_density*molar_mass)/avogadro_constant",
        "molarity*molar_mass",
        "mass_fraction*((pressure*molar_mass_dry_air)/(molar_gas_constant*temperature))",
    ],
    "molarity": [
        "mass_concentration/molar_mass",
        "mole_fraction*(pressure/(molar_gas_constant*temperature))",
        "number_density/avogadro_constant",
        "mass_fraction*(molar_mass_dry_air/molar_mass)*(pressure/(molar_gas_constant*temperature))",
    ],
    "number_density": [
        "mole_fraction*((avogadro_constant*pressure)/(molar_gas_constant*temperature))",
        "(mass_concentration*avogadro_constant)/molar_mass",
        "molarity*avogadro_constant",
        "mass_fraction*((avogadro_constant*pressure*molar_mass_dry_air)/(molar_gas_constant*temperature*molar_mass))",
    ],
    "backscattering_coefficient": ["volume_scattering_function*(2.*pi)"],
    "scattering_coefficient": ["volume_scattering_function*(4.*pi)"],
    "volume_scattering_function": [
        "backscattering_coefficient/(2.*pi)",
        "scattering_coefficient/(4.*pi)",
    ],
    "mass_concentration>mass_fraction--conversion_factor": [
        "((molar_gas_constant*temperature)/(pressure*molar_mass_dry_air))"
    ],
    "mass_concentration>molarity--conversion_factor": ["1./molar_mass"],
    "mass_concentration>mole_fraction--conversion_factor": [
        "((molar_gas_constant*temperature)/(molar_mass*pressure))"
    ],
    "mass_concentration>number_density--conversion_factor": [
        "avogadro_constant/molar_mass"
    ],
    "mass_fraction>mass_concentration--conversion_factor": [
        "((pressure*molar_mass_dry_air)/(molar_gas_constant*temperature))"
    ],
    "mass_fraction>molarity--conversion_factor": [
        "((pressure*molar_mass_dry_air)/(molar_gas_constant*temperature*molar_mass))"
    ],
    "mass_fraction>mole_fraction--conversion_factor": [
        "molar_mass_dry_air/molar_mass"
    ],
    "mass_fraction>number_density--conversion_factor": [
        "((avogadro_constant*pressure*molar_mass_dry_air)/(molar_gas_constant*temperature*molar_mass))"
    ],
    "molarity>mass_concentration--conversion_factor": ["molar_mass"],
    "molarity>mass_fraction--conversion_factor": [
        "((molar_gas_constant*temperature*molar_mass)/(pressure*molar_mass_dry_air))"
    ],
    "molarity>mole_fraction--conversion_factor": [
        "((molar_gas_constant*temperature)/pressure)"
    ],
    "molarity>number_density--conversion_factor": ["avogadro_constant"],
    "mole_fraction>mass_concentration--conversion_factor": [
        "((molar_mass*pressure)/(molar_gas_constant*temperature))"
    ],
    "mole_fraction>mass_fraction--conversion_factor": [
        "molar_mass/molar_mass_dry_air"
    ],
    "mole_fraction>molarity--conversion_factor": [
        "(pressure/(molar_gas_constant*temperature))"
    ],
    "mole_fraction>number_density--conversion_factor": [
        "((avogadro_constant*pressure)/(molar_gas_constant*temperature))"
    ],
    "number_density>mass_concentration--conversion_factor": [
        "molar_mass/avogadro_constant"
    ],
    "number_density>mass_fraction--conversion_factor": [
        "((molar_gas_constant*temperature*molar_mass_dry_air)/(avogadro_constant*molar_mass*pressure))"
    ],
    "number_density>molarity--conversion_factor": ["1./avogadro_constant"],
    "number_density>mole_fraction--conversion_factor": [
        "((molar_gas_constant*temperature)/(avogadro_constant*pressure))"
    ],
    "volume_scattering_function>backscattering_coefficient--conversion_factor": [
        "2.*pi"
    ],
    "volume_scattering_function>scattering_coefficient--conversion_factor": [
        "4.*pi"
    ],
    "backscattering_coefficient>volume_scattering_function--conversion_factor": [
        "1./(2.*pi)"
    ],
    "scattering_coefficient>volume_scattering_function--conversion_factor": [
        "1./(4.*pi)"
    ],
}
...
def _build_non_si_quantities(species, valence):
    """
    Build the non-SI-quantities dictionary for a given species/valence pair.

    This is the only part of the unit-converter reference data that depends on
    instance state (species-specific atom counts and valence-based factors), so
    unlike the rest of load_dictionaries() it cannot be a plain module constant.
    It is cached per (species, valence) pair instead, so it is only rebuilt once
    per distinct species encountered, rather than on every UnitConverter() call.
    """
    ...
    return {
        "amount of substance": {
            ...
            "mole of carbon": {
                "factor": 1.0 / get_N_atoms(species, "C"),
                "standard_units": "molC",
                "units": ["molC", "moleC"],
                "dependencies": ["species"],
            },
            "mole of chlorine": {
                "factor": 1.0 / get_N_atoms(species, "Cl"),
                "standard_units": "molCl",
                "units": ["molCl", "moleCl"],
                "dependencies": ["species"],
            },
            "mole of hydrogen": {
                "factor": 1.0 / get_N_atoms(species, "H"),
                "standard_units": "molH",
                "units": ["molH", "moleH"],
                "dependencies": ["species"],
            },
            "mole of nitrogen": {
                "factor": 1.0 / get_N_atoms(species, "N"),
                "standard_units": "molN",
                "units": ["molN", "moleN"],
                "dependencies": ["species"],
            },
            "mole of sulphur": {
                "factor": 1.0 / get_N_atoms(species, "S"),
                "standard_units": "molS",
                "units": ["molS", "moleS"],
                "dependencies": ["species"],
            },
        ...
        "mass": {
            ...
            "gram of carbon": {
                "factor": 1e-3
                * (
                    get_molecular_mass(species)
                    / (get_molecular_mass("C") * get_N_atoms(species, "C"))
                ),
                "standard_units": "gC",
            ...
            "gram of chlorine": {
                ...
                * (
                    get_molecular_mass(species)
                    / (get_molecular_mass("Cl") * get_N_atoms(species, "Cl"))
                ),
                "standard_units": "gCl",
            ...
            "gram of nitrogen": {
                ...
                * (
                    get_molecular_mass(species)
                    / (get_molecular_mass("N") * get_N_atoms(species, "N"))
                ),
                "standard_units": "gN",
            ...
            "gram of calcium carbonate": {
                "factor": 1e-3
                * (
                    (get_molecular_mass(species) / valence)
                    / (get_molecular_mass("CaCO3") / 2)
                ),  # valence of calcium carbonate is 2
            ...
            "gram of sulphur": {
                ...
                * (
                    get_molecular_mass(species)
                    / (get_molecular_mass("S") * get_N_atoms(species, "S"))
                ),
                "standard_units": "gS",
        ...
        "mole_fraction": {
            ...
            "ppmv of carbon": {
                "factor": 1e-6 / (get_N_atoms(species, "C")),
                "standard_units": "umolC mol-1",
                "units": [
                    "ppmvC",
                    "ppmC",
                    "partspermillioncarbon",
                    "PartspermillionCarbon",
                    "ppmVC",
                ],
                "dependencies": ["species"],
            ...
            "ppmv of chlorine": {
                "factor": 1e-6 / (get_N_atoms(species, "Cl")),
            ...
            "ppmv of nitrogen": {
                "factor": 1e-6 / (get_N_atoms(species, "N")),
            ...
            "ppmv of sulphur": {
                "factor": 1e-6 / (get_N_atoms(species, "S")),
            ...
            "ppbv of carbon": {
                "factor": 1e-9 / (get_N_atoms(species, "C")),
            ...
            "ppbv of chlorine": {
                "factor": 1e-9 / (get_N_atoms(species, "Cl")),
            ...
            "ppbv of nitrogen": {
                "factor": 1e-9 / (get_N_atoms(species, "N")),
            ...
            "ppbv of sulphur": {
                "factor": 1e-9 / (get_N_atoms(species, "S")),
            ...
            "pptv of carbon": {
                "factor": 1e-12 / (get_N_atoms(species, "C")),
            ...
            "pptv of chlorine": {
                "factor": 1e-12 / (get_N_atoms(species, "Cl")),
            ...
            "pptv of nitrogen": {
                "factor": 1e-12 / (get_N_atoms(species, "N")),
            ...
            "pptv of sulphur": {
                "factor": 1e-12 / (get_N_atoms(species, "S")),
        ...
        "pressure": {
            ...
            "bar": {"factor": 1e5, "standard_units": "bar", "units": ["bar"]},
            "torr": {
                "factor": 133.3224,
                "standard_units": "Torr",
                "units": ["Torr", "torr"],
            },
            "psi": {"factor": 6894.76, "standard_units": "psi", "units": ["psi"]},
        },
        "temperature": {
            "celsius": {
                "factor": 1.0,
...
# atomic masses per element in g mol-1, used by get_molecular_mass()
_ATOMIC_MASSES = {
    "H": 1.00794,
    "He": 4.002602,
    "Li": 6.941,
...
# chemical valence / absolute charge per molecule, used by get_valence()
_VALENCE_TABLE = {
...
class UnitConverter:
    ...
    def do_conversion(self):
        ...
        for prefix in self.standard_prefixes:
            standard_prefix_names.append(
                self.standard_prefixes[prefix]["standard_units"]
            )
        ...
        if self.conversion_type == "same_quantities":
            ...
            self.scaling_factor = (
                self.input_scaling_factor_to_reference
                / self.output_scaling_factor_to_reference
            )
            ...
            if self.output_quantity == "temperature":
                if (self.input_standard_units[0] not in standard_prefix_names) & (
                    self.output_standard_units[0] not in standard_prefix_names
                ):
                    self.correct_temperature(
                        self.input_value,
                        self.input_standard_units,
                        self.output_standard_units,
                    )
        ...
        # and then calculate output value using conversion formula
            ...
            for m_quantity in self.constants:
                self.SI_input_quantities[m_quantity] = self.constants[m_quantity][
                    "value"
                ]
                ...
                if quantity not in self.constants:
                    ...
                    # convert to base SI
                        ...
                        self.converted_value = (
                            self.input_value[quantity] * self.scaling_factor
                        )
            ...
            if self.user_input_quantity != "":
                ...
                self.conversion_factor = (
                    conversion_scaling_factor_1 * conversion_scaling_factor_2
                )
            ...
            if self.output_standard_units != self.output_reference_units:
                ...
                if self.user_input_quantity != "":
                    ...
                    self.conversion_factor = (
                        self.conversion_factor * conversion_scaling_factor_3
                    )
                ...
                if self.output_quantity == "temperature":
                    if self.output_standard_units[0] not in standard_prefix_names:
                        self.correct_temperature(
                            self.converted_value, "K", self.output_standard_units
                        )
        ...
        if self.precision != -999:
            ...
            try:
                self.conversion_factor = np.around(
                    self.conversion_factor, self.precision
                )
            except:
                pass
    ...
    def check_quantity_agreement(self):
        ...
        self.input_quantity = self.get_quantity_representation(
            self.input_reference_units,
            self.input_reference_simplified_units,
            "input",
            self.user_input_quantity,
        )
        self.output_quantity = self.get_quantity_representation(
            self.output_reference_units,
            self.output_reference_simplified_units,
            "output",
            self.user_output_quantity,
        )

        # are quantities represented same between input/output?
        if self.input_quantity != self.output_quantity:
            sys.exit(
                "Input and output quantities do not agree: '{}', '{}'. Check input and output units.".format(
    ...
    def check_conversion_validity(self):
        ...
        if set(list(self.input_units.keys())) != set(list(self.input_value.keys())):
            sys.exit(
                "Input unit and value dictionaries do not have same defined quantity names."
            )
    ...
    def get_quantity_representation(
        self, units, units_simplified, unit_type, unit_quantity
    ):
        """
        ...
        # find which known quantities (SI base, SI derived, or constants) have a
        # reference base_units signature matching either the units or their
        # simplified form, via a lookup precomputed once at import time instead
        # of rescanning every known quantity (and rebuilding Counter() objects
        # for each) on every call. Behaviourally equivalent to the previous
        # per-quantity `Counter(units) == Counter(base_units)` scan.
        units_signature = _canonical_unit_signature(units)
        simplified_signature = _canonical_unit_signature(units_simplified)

        if units_signature == simplified_signature:
            # bucket is already in original definition order, no reordering needed
            valid_quantities = list(
                _QUANTITY_SIGNATURE_LOOKUP.get(units_signature, [])
            )
        else:
            matched_quantities = set(
                _QUANTITY_SIGNATURE_LOOKUP.get(units_signature, [])
            ) | set(_QUANTITY_SIGNATURE_LOOKUP.get(simplified_signature, []))
            valid_quantities = [
                quantity
                for quantity in _QUANTITY_ORDER_NAMES
        ...
        # elif have more than 1 valid quantity, if user has given a input/output quantity, use that to select correct matching quantity
            ...
            # elif user has not provided a quantity the operation depends if are converting between the same quantity or not
                ...
                # if are converting between different quantities then throw error as the specific quantity is needed to be known for conversion
                    ...
                    sys.exit(
                        "Multiple valid quantities found for {} units: '{}'. Valid quantites are: {}. You must specify {}_quantity.".format(
                            unit_type, units, valid_quantities, unit_type
                        )
                    )
            else:
                sys.exit(
                    "Specified {}_quantity: '{}' not valid for {}_units: '{}'. Valid quantities are: {}.".format(
                        unit_type, unit_quantity, unit_type, units, valid_quantities
                    )
                )
    ...
    def get_conversion_formula(self, formula_type="standard"):
        ...
        if formula_type == "standard":
            ...
            available_conversion_formulae_quantities = list(
                self.conversion_formulae.keys()
            )

            # if don't have formula, return error message
            if self.output_quantity not in available_conversion_formulae_quantities:
                sys.exit(
                    "Do not have formula to calculate desired output quantity: '{}'.".format(
                ...
                for formula in quantity_formulae:
                    formula_quantities = (
                        formula.translate(remove_digits)
                        .replace("*", "$")
                        .replace("/", "$")
                        .replace("(", "")
                        .replace(")", "")
                    ...
                    formula_quantities = [
                        formula_quantity
                        for formula_quantity in formula_quantities
                        if formula_quantity
                    ]
                    # test to see if have all of formula quantities required in input quantities
                    have_quantities = set(formula_quantities).issubset(
                ...
                sys.exit(
                    "Do not have correct input quantities to calculate desired output quantity: '{}'.".format(
                        self.output_quantity
                    )
                )
        ...
        # is the conversion formula a factor (i.e. formula to get factor between an input and output quantity)
            ...
            if (
                "{}>{}--conversion_factor".format(
                    self.user_input_quantity, self.output_quantity
                )
                not in available_conversion_formulae_quantities
            ):
                sys.exit(
                    "Do not have defined formula to calculate conversion factor between input quantity: '{}' and output quantity: '{}'.".format(
                        self.user_input_quantity, self.output_quantity
                    )
                )
                ...
                # defined in conversion_formulae dictionary
                quantity_formulae = self.conversion_formulae[
                    "{}>{}--conversion_factor".format(
                        self.user_input_quantity, self.output_quantity
                    )
                ]
                ...
                # if do not valid input quantities for one of the formulae, return error message
                sys.exit(
                    "Do not have correct input quantities to calculate conversion factor between input quantity: '{}' and output quantity: '{}'.".format(
                        self.user_input_quantity, self.output_quantity
                    )
                )

    def correct_temperature(
        self, input_value, input_standard_unit, output_standard_unit
    ):
        """
    ...
    def standardise_unit_form(self, units, unit_type):
        ...
        if units == "":
            sys.exit(
                "The {} units are empty. If you want to set empty units, set it as: 'unitless'".format(
                    unit_type
                )
            )
        ...
        for element in ["C", "CaCO3", "Cl", "N", "S"]:
            ...
            units = units.replace(
                "mol {}/mol".format(element), "mol{}/mol".format(element)
            )
            units = units.replace(
                "mol {} mol-1".format(element), "mol{} mol-1".format(element)
            )
        ...
        for quantity in self.SI_base_quantities:
            for prefix in self.standard_prefixes:
                for prefix_variant in self.standard_prefixes[prefix]["units"]:
                    ...
                    if (
                        prefix_variant + self.SI_base_quantities[quantity]["base_units"]
                        in units
                    ):
                        match_list.append(
                            prefix_variant
                            + self.SI_base_quantities[quantity]["base_units"]
                        )
                        scaling_factors_to_reference.append(
                            self.standard_prefixes[prefix]["factor"]
                        )
                        reference_units.append(
                            self.SI_base_quantities[quantity]["base_units"]
                        )
                        standard_units.append(
                            self.standard_prefixes[prefix]["standard_units"]
                            + self.SI_base_quantities[quantity]["base_units"]
                        )
                ...
                for prefix in self.standard_prefixes:
                    for prefix_variant in self.standard_prefixes[prefix]["units"]:
                        ...
                        if prefix_variant + equiv_unit in units:
                            ...
                            reference_units.append(
                                self.SI_derived_quantities[quantity]["base_units"]
                            )
                            standard_units.append(
                                self.standard_prefixes[prefix]["standard_units"]
                                + self.SI_derived_quantities[quantity]["standard_units"]
                            )
                            non_SI_unit_dependencies.append([])
                    ...
                    for prefix in self.standard_prefixes:
                        ...
                        for prefix_variant in self.standard_prefixes[prefix]["units"]:
                            ...
                            if prefix_variant + unit in units:
                                ...
                                scaling_factors_to_reference.append(
                                    self.non_SI_quantities[quantity][quantity_variant][
                                        "factor"
                                    ]
                                    * self.standard_prefixes[prefix]["factor"]
                                )
                                standard_units.append(
                                    self.standard_prefixes[prefix]["standard_units"]
                                    + self.non_SI_quantities[quantity][
                                        quantity_variant
                                    ]["standard_units"]
                                )
                                try:
                                    ...
                                    reference_units.append(
                                        self.SI_derived_quantities[quantity][
                                            "base_units"
                                        ]
                                    )
                                if (
                                    "dependencies"
                                    in self.non_SI_quantities[quantity][
                                        quantity_variant
                                    ]
                                ):
                                    non_SI_unit_dependencies.append(
        ...
        if len(match_list) == 0:
            sys.exit(
                "The {} units: '{}' are not recognised. This may be due to incorrect capitalisations or spacings.".format(
                    unit_type, units
                )
            )
        ...
        for mm, match_unit in enumerate(match_list):
            ...
            if match_unit in cut_units:
                ...
                match_inds = [
                    ...
                    if units.startswith(match_unit, i)
                ]
                ...
                for match_ind in match_inds:
                    ...
                    if not any(match_ind in x for x in accepted_unit_inds_in_str):
                        relevant_inds = list(
                            range(match_ind, match_ind + len(match_unit))
                        )
                        ...
                        while True:
                            ...
                            if next_relevant_ind >= len(units):
                                break
                            if (units[next_relevant_ind] == "-") or (
                                units[next_relevant_ind].isdigit()
                            ):
                        ...
                        unit_unmatched_inds = sorted(
                            [x for x in unit_unmatched_inds if x not in relevant_inds]
                        )
                        ...
                        relevant_units = {
                            "clean": match_unit_with_exponent,
                            "reference": reference_units[mm],
                            "standard": standard_units[mm],
                        }
                        ...
                        if ("species" in dependencies) & ("valence" in dependencies):
                            if (self.species == "") & (np.isnan(self.valence)):
                                sys.exit(
                                    " The {} units: '{}' requires both 'species' and 'valence' to be passed as input arguments.".format(
                                        unit_type, units
                                    )
                                )
                            ...
                            if self.species == "":
                                sys.exit(
                                    "The {} units: '{}' requires 'species' to be passed as an input argument.".format(
                                        unit_type, units
                                    )
                                )
                            ...
                            if np.isnan(self.valence):
                                sys.exit(
                                    "The {} units: '{}' requires 'valence' to be passed as an input argument.".format(
                                        unit_type, units
                                    )
                                )
                        ...
                        # if only some of relevant indices are in denominator indices, then throw error as all of them should be
                            ...
                            sys.exit(
                                "The {} units: '{}' syntax is invalid.".format(
                                    unit_type, units
                                )
                            )
                        ...
                        for relevant_unit_key in ["clean", "reference", "standard"]:
                            ...
                            if current_exponent != "":
                                if relevant_unit_key == "clean":
                                    ...
                                    new_exponent = (
                                        int(current_exponent) * match_unit_exponent
                                    )
                                # if new exponent != 1, then add it
                                if new_exponent != 1:
                                    new_unit += str(new_exponent)
                        ...
                        relevant_scaling_factor_to_reference = (
                            scaling_factors_to_reference[mm]
                        )
        ...
        if (cut_units != "") & (not cut_units.isspace()):
            sys.exit(
                "Parts of the {} units: '{}' are not recognised. This may be due to incorrect capitalisations or spacings.".format(
                    unit_type, units
                )
            )
    ...
    def load_dictionaries(self):
        """
        Loads reference dictionaries used for unit parsing and conversion.

        The SI/unit-system dictionaries are static and shared across every
        instance, so they are assigned directly from the module-level constants
        built once at import time. non_SI_quantities is the one exception, since
        it depends on this instance's species/valence; it is built via a cached
        helper so it is only rebuilt once per distinct (species, valence) pair.
        """

        self.standard_prefixes = _STANDARD_PREFIXES
        self.SI_base_quantities = _SI_BASE_QUANTITIES
        self.SI_derived_quantities = _SI_DERIVED_QUANTITIES
        self.constants = _CONSTANTS
        self.non_SI_quantities = _build_non_si_quantities(self.species, self.valence)
        self.conversion_formulae = _CONVERSION_FORMULAE
...
def get_N_atoms(molecule, element):
    ...
    parse_list = re.findall(
        r"[A-Z][a-z]*|\d+", re.sub(r"[A-Z][a-z]*(?![\da-z])", r"\g<0>1", molecule)
    )
