...
from .download_aux import (
    find_model,
    find_available_resolutions,
    find_available_species,
    find_available_nc_files,
    build_nc_file_paths_in_range,
    download_non_interpolated_sftp,
)
...
class Download(object):
        ...
        for section_ind, section in enumerate(self.sections):
                ...
                if self.network and self.dl_mode != "mod":
                    ...
                    if self.reading_ghost:
                        if not self.dl_ghost_source:
                            ...
                            self.dl_ghost_source = (
                                "bsc" if dl_ghost_source in ["", "y"] else "zenodo"
                            )
                        else:
                            # exit if the value is neither bsc or zenodo
                            if str(self.dl_ghost_source).lower() not in [
                                "bsc",
                                "zenodo",
                            ]:
                    ...
                    combined_networks = [
                        (network, None) for network in self.network
                    ] + [
                        (network_specie.split("|")[0], network_specie.split("|")[1])
                        for network_specie in self.filter_species
                    ]
                    ...
                    download_fun = (
                        self.download_ghost_network_sftp
                        if self.reading_ghost and self.dl_ghost_source == "bsc"
                        else self.zenodo.download_ghost_network_zenodo
                        if self.reading_ghost
                    ...
                    for network, filter_species in combined_networks:
                        ...
                        # GHOST and non-GHOST
                            ...
                            initial_check_nc_files = download_fun(
                                network, initial_check=True
                            )
            ...
            if self.experiments and self.dl_mode != "obs":
                ...
                # local model download
                    ...
                    download_model_fun = (
                        self.download_model
                        if self.dl_interpolated
                        else self.download_non_interpolated_model
                    )
                ...
                for model in self.experiments:
                    ...
                    if model.startswith(tuple(cams_options.keys())):
                        self.cams = Cams(self)
                        initial_check_nc_files = self.cams.download_cams_model(
                    ...
                    # BSC machines
                        ...
                        initial_check_nc_files = download_model_fun(
                            model, initial_check=True
                        )
                        files_to_download = self.select_files_to_download(
                            initial_check_nc_files
    ...
    def connect(self):
        ...
        self.mod_to_interp_remote_path = data_paths[self.remote_machine][
            "mod_to_interp_root"
        ]
            ...
            if type(nc_filepaths_to_download) is dict:
                for dir, dir_dict in nc_filepaths_to_download.items():
                    ...
                    downloaded_before_execution_files = list(
                        filter(
                            lambda x: self.prov_start_time
                            > os.path.getctime(join(dir, x)),
                            downloaded_files,
                        )
                    )
                ...
                not_downloaded_paths = list(
                    filter(lambda x: not os.path.exists(x), nc_filepaths_to_download)
                )
                downloaded_files = list(
                    filter(lambda x: os.path.exists(x), nc_filepaths_to_download)
                )

                # get the files that were downloaded before the execution
                downloaded_before_execution_files = list(
                    filter(
                        lambda x: self.prov_start_time > os.path.getctime(x),
                        downloaded_files,
                    )
                )
    ...
    def download_nonghost_network(self, network, initial_check, files_to_download=None):
        ...
        not_available_resolutions = set(self.resolution) - set(
            self.nonghost_available_resolutions
        )
        ...
        sftp_resolutions = (
            ...
            if self.resolution
                ...
                self.sftp.listdir(join(self.nonghost_remote_obs_path, network))
            ).intersection(self.nonghost_available_resolutions)
        )
            ...
            try:
                sftp_species = (
                    ...
                    if self.species
                        ...
                        self.sftp.listdir(
                            join(self.nonghost_remote_obs_path, network, resolution)
                        )
                    ).intersection(self.available_species)
                )
            except FileNotFoundError:
            ...
            for remote_dir in res_spec_dir:
                ...
                # download the valid resolution specie date combinations
                    ...
                    if not initial_check:
                        ...
                        valid_nc_files = list(
                            filter(
                                lambda x: join(local_dir, x) in files_to_download,
                                valid_nc_files,
                            )
                        )
                        if not valid_nc_files:
                    ...
                    for nc_file in valid_nc_files_iter:
                        ...
                        if initial_check:
                            ...
                            self.sftp.get(
                                remote_path, local_path, callback=self.check_time
                            )

            return initial_check_nc_files

    def download_ghost_network_sftp(
        self, network, initial_check, files_to_download=None
    ):
        """
        ...
        # if not valid combination of GHOST version and network, next
        elif self.ghost_version not in self.sftp.listdir(
            join(self.ghost_remote_obs_path, network)
        ):
            ...
            available_ghost_versions = set(
                self.sftp.listdir(join(self.ghost_remote_obs_path, network))
            ).intersection(self.possible_ghost_versions)
            ...
            if available_ghost_versions:
                ...
                for possible_ghost_version in available_ghost_versions:
                    remote_dir_ghost_version = join(
                        self.ghost_remote_obs_path, network, possible_ghost_version
                    )
                    ...
                    sftp_resolutions = (
                        ...
                        if self.resolution
                            ...
                            self.sftp.listdir(remote_dir_ghost_version)
                        ).intersection(self.ghost_available_resolutions)
                    )
                    for resolution in sftp_resolutions:
                        try:
                            species_list = (
                                self.species
                                if self.species
                                else self.sftp.listdir(
                                    join(remote_dir_ghost_version, resolution)
                                )
                            )
                        except FileNotFoundError:
                            continue
                        ...
                        for species in species_list:
                            ...
                            try:
                                nc_files = self.sftp.listdir(
                                    join(remote_dir_ghost_version, resolution, species)
                                )
                                valid_nc_files = self.get_valid_nc_files_in_date_range(
                                    nc_files
                                )
                                if valid_nc_files:
                                    valid_available_ghost_versions.append(
                                        possible_ghost_version
                                    )
                                    break
                            except FileNotFoundError:
                                continue
        ...
        sftp_resolutions = (
            self.resolution
            if self.resolution
            else set(self.sftp.listdir(remote_dir)).intersection(
                self.ghost_available_resolutions
            )
        )
        for resolution in sftp_resolutions:
            try:
                sftp_species = (
                    ...
                    if self.species
                        ...
                        self.sftp.listdir(join(remote_dir, resolution))
                    ).intersection(self.available_species)
                )
            except FileNotFoundError:
                msg = f"There is no data available in {self.remote_machine} for {network} network at {resolution} resolution."
    ...
    def download_model(self, model, initial_check, files_to_download=None):
        ...
        model = (
            model_old if self.ghost_version in ["1.2", "1.3", "1.3.1"] else model_new
        )
        ...
        try:
            ...
            possible_ghost_versions = set(
                self.sftp.listdir(self.mod_remote_path)
            ).intersection(set(self.possible_ghost_versions))
                        ...
                        for species in species_list:
                            ...
                            for network in network_list:
                                ...
                                try:
                                    ...
                                    valid_nc_files = (
                                        self.get_valid_nc_files_in_date_range(nc_files)
                                    )
                                    if valid_nc_files:
                                        valid_available_ghost_versions.append(
                                            possible_ghost_version
            ...
            for species in sftp_species:
                try:
                    sftp_network = (
                        self.network
                        if self.network
                        else self.sftp.listdir(join(remote_dir, resolution, species))
                    )
                except FileNotFoundError:
    ...
    def copy_non_interpolated_model(self, model, initial_check, files_to_download=None):
        ...
        sftp_resolutions = (
            self.model_resolution
            if self.model_resolution
            else set(os.listdir(esarchive_dir)).intersection(
                self.nonghost_available_resolutions
            )
        )
        ...
        for resolution in sftp_resolutions:
            try:
                ...
                available_species = self.available_species + [
                    spec[0] for spec in mapping_species.values()
                ]
                sftp_species = (
                    self.species
                    if self.species
                    else set(os.listdir(join(esarchive_dir, resolution))).intersection(
                        available_species
                    )
                )
            except FileNotFoundError:
            ...
            for esarchive_dir in res_spec_dir:
                ...
                if nc_files:
                    ...
                    if ensemble.isdigit() or ensemble == "allmembers":
                        ...
                        formats_list = [
                            (file.count("-"), file.count("_")) for file in nc_files
                        ]
                        number_of_formats_dict = {
                            format: formats_list.count(format)
                            for format in set(formats_list)
                        }
                        format = max(
                            number_of_formats_dict, key=number_of_formats_dict.get
                        )
                        ...
                        if format == (0, 1):
                            ...
                            if ensemble == "000" or ensemble == "allmembers":
                                nc_files = list(
                                    filter(
                                        lambda x: x.split("_")[0] == species, nc_files
                                    )
                                )
                        ...
                        # example: od550du_2018011700_av_an.nc
                            ...
                            error = (
                                "It is not possible to copy this nc file type yet. Please, contact the developers.",
                                nc_files,
                            )
                            self.logger.error(error)
                            sys.exit(1)
                ...
                # copy the valid resolution specie date combinations
                    ...
                    if ensemble.isdigit() or ensemble == "allmembers":
                        gpfs_dir = join(
                            self.mod_to_interp_root, mod_id, domain, resolution, species
                        )
                    else:
                        gpfs_dir = join(
                            self.mod_to_interp_root,
                            mod_id,
                            domain,
                            resolution,
                            "ensemble-stats",
                    ...
                    if not os.path.exists(gpfs_dir):
                        ...
                        for i in range(4):
                            os.system(
                                f"chmod 770 {temp_gpfs_dir}; chgrp bsc32 {temp_gpfs_dir}"
                            )
                    ...
                    if not initial_check:
                        ...
                        valid_nc_files = list(
                            filter(
                                lambda x: join(gpfs_dir, x) in files_to_download,
                                valid_nc_files,
                            )
                        )
                        if not valid_nc_files:
                    ...
                    if not initial_check and not self.logfile:
                        ...
                        valid_nc_files_iter = tqdm(
                            valid_nc_files,
                            bar_format="{l_bar}{bar}|{n_fmt}/{total_fmt}",
                            desc=f"    Copying files from esarchive to gpfs ({len(valid_nc_files)})",
                        )
                    else:
                        # do not print the bar if it is the initial check
                        valid_nc_files_iter = valid_nc_files
                    ...
                    for nc_file in valid_nc_files_iter:
                        ...
                        if initial_check:
                            ...
                            rsync_command = (
                                "dtrsync" if self.machine == "storage5" else "rsync"
                            )
                            ...
                            try:
                                with open(os.devnull, "wb") as devnull:
                                    subprocess.check_call(
                                        [rsync_command, esarchive_path, gpfs_path],
                                        stdout=devnull,
    ...
    def get_all_models(self):
        ...
        if self.dl_interpolated is True:
            ...
            model_list = self.sftp.listdir(
                join(self.mod_remote_path, self.ghost_version)
            )
