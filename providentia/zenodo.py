...
zenodo_dois = yaml.safe_load(
    open(join(PROVIDENTIA_ROOT, "settings", "internal", "zenodo", "zenodo_dois.yaml"))
)
...
class Zenodo:
    ...
    def __init__(self, download_instance):
        ...
        if self.download_instance.ghost_version not in zenodo_dois:
            error = (
                f"Error: Current GHOST version ({self.download_instance.ghost_version}) is not available on Zenodo. "
        ...
        self.artifact_mapping = yaml.safe_load(
            open(
                join(
                    PROVIDENTIA_ROOT,
                    "settings",
                    "internal",
                    "zenodo",
                    f"zenodo_{self.download_instance.ghost_version}.yaml",
                )
            )
        )
    ...
    def download_zip(self, network, artifact_network):
        ...
        with self.get_zip() as r:
            ...
            with open(zip_path, "wb") as f, tqdm(
                total=total,
                unit="B",
                desc=f"    Downloading {network}.zip",
                unit_scale=True,
            ) as pbar:
    ...
    def extract_tar(self, files_info):
        ...
        for dir, file_info_dict in files_info.items():
            ...
            for filename in tqdm(
                file_info_dict["filenames"],
                bar_format="{l_bar}{bar}|{n_fmt}/{total_fmt}",
                desc=f"      TAR extraction in progress... ({len(file_info_dict['filenames'])})",
            ):
                # open tar file
    ...
    def check_filetrees(self, network):
        ...
        filetree_path = join(
            ...
            "internal",
            "zenodo",
            f"zenodo_ghost_filetree_{self.download_instance.ghost_version}.json",
        )
        ...
        valid_resolutions = (
            self.download_instance.resolution
            if self.download_instance.resolution != ""
            else zenodo_filetrees[network].keys()
        )
        for resolution in valid_resolutions:
            if resolution in zenodo_filetrees[network]:
                # determine which species to process
                valid_species = (
                    self.download_instance.species
                    if self.download_instance.species != ""
                    else zenodo_filetrees[network][resolution].keys()
                )
            ...
            for resolution, species_dict in resolutions.items():
                for species, dates in species_dict.items():
                    for date in dates:
                        ...
                        path = join(
                            self.download_instance.ghost_root,
                            network,
    ...
    def download_ghost_network_zenodo(
        ...
        if initial_check:
            ...
            valid_files_info = self.extract_zip(
                files_to_download, zip_path, initial_check
            )
