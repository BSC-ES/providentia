...
class Plotting:
    ...
    def __init__(self, read_instance=None, canvas_instance=None):
        ...
        proj_class = getattr(
            ccrs,
            self.canvas_instance.plot_characteristics_templates["map"]["projection"],
        )
        ...
        for plot_type in plot_types:
            ...
            if self.read_instance.mode in ["report", "library"]:
                ...
                # if no models are defined, remove all bias plots, or plots with bias statistics
                    ...
                    if len(data_labels) == 1:
                        msg = (
                            f"No models defined, so {plot_type} plot cannot be created."
                        )
                        show_message(self.read_instance, msg)
                        valid_plot_type = False
            ...
            if zstat:
                ...
                stats_dict = {
                    ...
                    **self.read_instance.modbias_stats,
                }
                ...
                if self.read_instance.mode in ["report", "library"]:
                    ...
                    # warning for timeseries bias plot if the temporal colocation is not active
                        ...
                        ("timeseries" == base_plot_type)
                        & ("bias" in plot_options)
                        & (not self.read_instance.temporal_colocation)
                    ):
                    ...
                    if not valid_plot_type:
                        if self.read_instance.mode == "report":
                            if plot_type in self.read_instance.summary_plots_to_make:
                                self.read_instance.summary_plots_to_make.remove(
                                    plot_type
                                )
                            if plot_type in self.read_instance.station_plots_to_make:
                                self.read_instance.station_plots_to_make.remove(
                                    plot_type
                                )
                        elif self.read_instance.mode == "library":
                            return valid_plot_type
                ...
                if "page_title" in self.canvas_instance.plot_characteristics[plot_type]:
                    if (
                        "t"
                        in self.canvas_instance.plot_characteristics[plot_type][
                            "page_title"
                        ].keys()
                    ):
            ...
            # add new keys for plots without stats
                ...
                if self.read_instance.mode in ["report", "library"]:
                    ...
                    # warning for scatter and fairmode plots if have no models
                        ...
                        base_plot_type
                        in ["scatter", "fairmode-target", "fairmode-statsummary"]
                    ) & (len(data_labels) == 1):
                        msg = f"No models defined, so {plot_type} cannot be created."
                        show_message(self.read_instance, msg)
                        valid_plot_type = False
                    ...
                    elif ("periodic-violin" == base_plot_type) & (
                        self.read_instance.active_resolution == "annual"
                    ):
                        msg = f"{plot_type} cannot be created when active temporal resolution is annual"
                        show_message(self.read_instance, msg)
            ...
            if "orientation" in self.canvas_instance.plot_characteristics[plot_type]:
                if (
                    self.canvas_instance.plot_characteristics[plot_type]["orientation"]
                    == "landscape"
                ):
                    self.canvas_instance.plot_characteristics[plot_type]["figure"][
                        "figsize"
                    ] = self.canvas_instance.landscape_figsize
                elif (
                    self.canvas_instance.plot_characteristics[plot_type]["orientation"]
                    == "portrait"
                ):
                    self.canvas_instance.plot_characteristics[plot_type]["figure"][
                        "figsize"
                    ] = self.canvas_instance.portrait_figsize
        ...
        for model in data_labels:
            if model != self.read_instance.observations_data_label:
                ...
                grid_edge_outline_poly = Polygon(
                    np.vstack(
                        (
                            self.read_instance.plotting_params[model][
                                "grid_edge_longitude"
                            ],
                            self.read_instance.plotting_params[model][
                                "grid_edge_latitude"
    ...
    def draw_line(self, fig, key, value, plot_characteristics, y, spacing, max_len):
        ...
        # add key
        fig.text(
            y=y + (n_lines - 1) * spacing, s=f"{key}:", **plot_characteristics["keys"]
        )
    ...
    def make_header(self, pdf, plot_characteristics):
        ...
        if plot_characteristics["dark_mode"]:
            ...
            for key in ["keys", "values"]:
                plot_characteristics[key]["color"] = plot_characteristics[key]["color"][
                    "dark_mode"
                ]
        ...
        start_date = datetime.strptime(
            self.read_instance.start_date, "%Y%m%d"
        ).strftime("%d/%m/%Y")
        end_date = datetime.strptime(self.read_instance.end_date, "%Y%m%d").strftime(
            "%d/%m/%Y"
        )
        ...
        variable_values = {
            ...
            "filter_species": self.format_value(
                list(self.read_instance.filter_species.keys())
            ),
        ...
        with PdfPages(reports_doi_path_temp) as pdf:
            ...
            for subsection in self.canvas_instance.station_reference_data:
                for networkspeci in self.read_instance.networkspecies:
                    ...
                    reference_data = self.canvas_instance.station_reference_data[
                        subsection
                    ][networkspeci]
                    doi_data = self.canvas_instance.station_doi_data[subsection][
                        networkspeci
                    ]
                    ...
                    for station_reference, station_doi in zip(reference_data, doi_data):
                        ...
                        unique_station_references = list(np.unique(station_reference))
                        ...
                        unique_station_dois = list(np.unique(station_doi))
    ...
    def make_metadata(
        ...
        n_stations = self.canvas_instance.selected_station_metadata[networkspeci].shape[
            0
        ]
        ...
        if n_stations == 1:
            ...
            station_references = self.canvas_instance.selected_station_metadata[
                networkspeci
            ]["station_reference"].flatten()
            station_reference = station_references[
                ~pd.isnull(station_references)
            ].astype(str)[0]
        ...
        for ghost_var, ghost_var_dict in plot_characteristics[char_var].items():
            ...
            if "dp" in ghost_var_dict:
                str_to_plot += "{}: {:.{}f}".format(
                    ghost_var_dict[var_str_name],
                    np.nanmedian(
                        self.canvas_instance.selected_station_metadata[networkspeci][
                            ghost_var
                        ].astype(np.float32)
                    ),
                    ghost_var_dict["dp"],
            ...
            # if str then get unique elements or percentage dependent on n uniques
                ...
                all_current_meta = self.canvas_instance.selected_station_metadata[
                    networkspeci
                ][ghost_var].flatten()
                # remove NaNs
                all_current_meta = all_current_meta[
                    ~pd.isnull(all_current_meta)
                ].astype(str)

                # get counts of all unique metadata elements across selected stations
                unique_meta, meta_counts = np.unique(
                    all_current_meta, return_counts=True
                )
                ...
                # 1 unique metadata element? then return it
                    ...
                    str_to_plot += "{}: {}".format(
                        ghost_var_dict[var_str_name], unique_meta[0]
                    )
                # if have > N unique metadata elements, just return count of the elements across the selected stations
                    ...
                    str_to_plot += "{}: {} uniques".format(
                        ghost_var_dict[var_str_name], n_unique_meta
                    )
                # otherwise, get percentage of unique metadata elements across selected stations
                else:
                    meta_pc = (100.0 / len(all_current_meta)) * meta_counts
                    ...
                    str_to_plot += "{}: {}".format(
                        ...
                        ", ".join(
                            [
                                ":".join([str(var), pc])
                                for var, pc in zip(unique_meta, meta_pc)
                            ]
                        ),
                    )
        ...
        if self.read_instance.mode == "report":
            ...
            ax_width_px = (
                relevant_axis.bbox.width * plot_characteristics["figure"]["nrows"]
            )
            ...
            ax_bbox = relevant_axis.get_window_extent().transformed(
                self.canvas_instance.figure.dpi_scale_trans.inverted()
            )
    ...
    def make_timeseries(
        ...
        if (self.read_instance.mode in ["report", "library"]) and (
            "hidedata" in plot_options
        ):
            return
        ...
        if self.read_instance.mode not in ["report", "library"]:
            ...
            chunk_resolution = (
                self.canvas_instance.timeseries_chunk_resolution.currentText()
            )
        ...
        if (chunk_stat is not None) and (chunk_resolution is not None):
            timeseries_data = create_statistical_timeseries(
                self.read_instance,
                self.canvas_instance,
                chunk_stat,
                chunk_resolution,
        ...
        # normal timeseries
            ...
            timeseries_data = self.canvas_instance.selected_station_data[networkspeci][
                "timeseries"
            ]
        ...
        if add_bias_line:
            ...
            bias_line_chars = {
                k: v for k, v in plot_characteristics["bias_line"].items() if k != "y"
            }
            bias_line = relevant_axis.axhline(y=mb, **bias_line_chars)
            # track plot elements
            if self.read_instance.mode not in ["report"]:
                self.track_plot_elements(
                    "ALL", "timeseries", "bias_line", [bias_line], bias=bias
                )
    ...
    def make_periodic(
        ...
        for (
            nonrelevant_temporal_resolution
        ) in self.read_instance.periodic_nonrelevant_temporal_resolutions:
            # get subplot axis
        ...
        for (
            ...
            if zstat is None:
                ...
                period_x_grid, PDFs_sampled = self.make_distribution(
                    relevant_axis,
                    networkspeci,
                    data_labels,
                    plot_characteristics,
                ...
                for data_label_ii, data_label in enumerate(cut_data_labels):
                    ...
                    # get median zorder
                    median_zorder = self.read_instance.plotting_params[data_label][
                        "zorder"
                    ] + len(cut_data_labels)
                    ...
                    # make plot of median
                    median_plots = relevant_sub_ax.plot(
                        self.canvas_instance.unique_periods,
                        medians[:, data_label_ii],
                        color=self.read_instance.plotting_params[data_label]["colour"],
                    ...
                    for period_ii in range(len(self.canvas_instance.unique_periods)):
                        ...
                        if not np.all(np.isnan(PDF_sampled)):
                            PDF_sampled = (
                                0.5
                                * plot_characteristics["violin_widths"]
                                * PDF_sampled
                                / PDF_sampled.max()
                            )
                            ...
                            if violin_fill:
                                self.violin_plot = relevant_sub_ax.fill_betweenx(
                                    x_grid,
                                    -PDF_sampled + period_pos,
                                    PDF_sampled + period_pos,
                                    facecolor=self.read_instance.plotting_params[
                                        data_label
                                    ]["colour"],
                                    alpha=alpha,
                                ...
                                self.violin_plot = relevant_sub_ax.fill_betweenx(
                                    ...
                                    PDF_sampled + period_pos,
                                    facecolor="None",
                                    edgecolor=self.read_instance.plotting_params[
                                        data_label
                                    ]["colour"],
                                    **plot_characteristics["plot"]["violin"],
                                )
                            ...
                            if (
                                ...
                                and ("individual" not in plot_options)
                            ):
                                m = np.mean(
                                    self.violin_plot.get_paths()[0].vertices[:, 0]
                                )
                                ...
                                if (
                                    ...
                                    self.violin_plot.get_paths()[0].vertices[
                                        :, 0
                                    ] = np.clip(
                                        self.violin_plot.get_paths()[0].vertices[:, 0],
                                        -np.inf,
                    ...
                    if len(violins) > 0:
                        limit_plot = relevant_sub_ax.plot(
                    ...
                    if self.read_instance.mode not in ["report"]:
                        self.track_plot_elements(
                            data_label,
                            "periodic-violin",
                            "violin_plot_{}".format(relevant_temporal_resolution),
                            violins,
                            bias=False,
                        )
                        self.track_plot_elements(
                            data_label,
                            "periodic-violin",
                            "Median_plot_{}".format(relevant_temporal_resolution),
                            median_plots,
                            bias=False,
                        )
            ...
            # periodic plot type
                ...
                if z_statistic_sign == "bias":
                    ...
                    if z_statistic_type == "basic":
                        minimum_bias = self.read_instance.basic_stats[base_zstat][
                            "minimum_bias"
                        ]
                    else:
                        minimum_bias = self.read_instance.modbias_stats[base_zstat][
                            "minimum_bias"
                        ]
                    bias_lines = []
                    for mb in minimum_bias:
                        bias_lines += [
                            relevant_sub_ax.axhline(
                                y=mb, **plot_characteristics["bias_line"]
                            )
                        ]
                    # track plot elements
                    if self.read_instance.mode not in ["report"]:
                        self.track_plot_elements(
                            "ALL",
                            "periodic",
                            "bias_line_{}".format(relevant_temporal_resolution),
                            bias_lines,
                            bias=bias,
                        )
                ...
                for data_label_ii, data_label in enumerate(cut_data_labels):
                    ...
                    if (z_statistic_sign == "bias") & (
                        data_label == self.read_instance.observations_data_label
                    ):
                        continue

                    # make plot
                    self.periodic_plots = relevant_sub_ax.plot(
                        self.canvas_instance.unique_periods,
                        stats_calc[:, data_label_ii],
                        color=self.read_instance.plotting_params[data_label]["colour"],
                    ...
                    if self.read_instance.mode not in ["report"]:
                        self.track_plot_elements(
                            data_label,
                            "periodic",
                            "plot_{}".format(relevant_temporal_resolution),
                            self.periodic_plots,
                            bias=bias,
                        )
    ...
    def make_distribution(
        ...
        if violin_resolution is not None:
            PDFs_sampled = np.full(
                (
                    len(cut_data_labels),
                    len(self.canvas_instance.periodic_xticks[violin_resolution]),
                    int(n_samples),
                ),
                np.nan,
                dtype=np.float32,
        ...
        for data_label_ii, data_label in enumerate(cut_data_labels):
            ...
            if bias:
                ...
                if data_label_ii == 0:
                    kde_data_obs = drop_nans(
                        self.canvas_instance.selected_station_data[networkspeci][
                            "flat"
                        ][
                            valid_data_labels.index(
                                self.read_instance.observations_data_label
                            ),
                            0,
                            :,
            ...
            # setup standard plot
                ...
                if (violin_resolution is not None) & (data_label_ii == 0):
                    ...
                    for group in grouped_data:
                        ...
                        period_x_grid.append(
                            np.linspace(min_data, upper_inner_fence, int(n_samples))
                        )

                # get data (flattened and drop NaNs)
                if violin_resolution is not None:
                    kde_data_grouped = [
                        drop_nans(group[valid_data_labels.index(data_label)].flatten())
                        for group in grouped_data
                    ]
                else:
                    kde_data_grouped = [
    ...
    def make_scatter(
        ...
        for data_label in cut_data_labels:
            ...
            # create scatter plot
            self.scatter_plot = relevant_axis.plot(
                observations_data,
                model_data,
                color=self.read_instance.plotting_params[data_label]["colour"],
                **plot_characteristics["plot"],
            ...
            if self.read_instance.mode not in ["report"]:
                self.track_plot_elements(
                    data_label, "scatter", "plot", self.scatter_plot, bias=False
                )
    ...
    def make_boxplot(
        ...
        if np.all([True if "vconcaerobin" in speci else False for speci in species]):
            sizedist = True
            radii_bins = np.array(
                [get_AERONET_sizedist_bin_radius(speci) for speci in species]
            ).astype(np.float32)
        ...
        if "normalise" in plot_options:
            ...
            for data_label in data_labels:
                ...
                for ns in networkspecies:
                    # get valid data labels for networkspeci
                    valid_data_labels = (
                        self.canvas_instance.selected_station_data_labels[ns]
                    )
                    ...
                    # get data (flattened and drop NaNs)
                    data_array = drop_nans(
                        self.canvas_instance.selected_station_data[ns]["flat"][
                            valid_data_labels.index(data_label), 0, :
                        ]
                    )
        ...
        for ns in networkspecies:
            ...
            if len(cut_data_labels) > 0:
                ...
                if (
                    ...
                    available_width = plot_characteristics["group_widths"][
                        "multispecies"
                    ]
                    remainder_width = 1.0 - available_width
                    start_point = -0.5 + (remainder_width / 2.0)
                    widths = available_width / (len(cut_data_labels) + 0.15)
                    spacing = (available_width - (widths * len(cut_data_labels))) / (
                        len(cut_data_labels) - 1
                    )
                ...
                if (
                    ...
                    positions = [
                        (
                            (start_point + (widths / 2.0))
                            + (spacing * data_label_ii)
                            + (widths * data_label_ii)
                        )
                        + ns_current
                ...
                for data_label_ii, data_label in enumerate(cut_data_labels):
                    ...
                    # make boxplot
                    boxplot = relevant_axis.boxplot(
                        x=data_array,
                        positions=[positions[data_label_ii]],
                        widths=widths,
                        **plot_characteristics["plot"],
                    )
    ...
    def make_heatmap(
        ...
        if "annotate" in plot_options:
            ...
            if Version(pd.__version__) >= Version("2.1.0"):
                annotate = stats_df.map(
                    lambda x: round_decimal_places(x, decimal_places)
                )
            else:
                annotate = stats_df.applymap(
                    lambda x: round_decimal_places(x, decimal_places)
                )
        else:
            annotate = False
        ...
        if "multispecies" in plot_options:
            ...
            if (len(subsections) > 1) and (plotting_paradigm == "summary"):
                ...
                relevant_axis.hlines(
                    y=y_separators,
                    xmin=xmin,
                    xmax=0,
                    clip_on=False,
                    **plot_characteristics["multispecies"]["hlines"],
                )
                ...
                for networkspeci, y_separator in zip(networkspecies, y_separators[:-1]):
                    ...
                    if Version(matplotlib.__version__) >= Version("3.3"):
                        relevant_axis.annotate(
                            text=networkspeci_label,
    ...
    def make_taylor(
        ...
        tmin, tmax, smin, smax, gl1, tf1 = get_taylor_diagram_ghelper_info(
            stddev_max, plot_characteristics, extend
        )
        ...
        if self.read_instance.mode not in ["report", "library"]:
            ...
            for plot_position in range(2, 6):
                plot_type = getattr(
                    self.read_instance, "position_{}".format(plot_position)
                )
                if plot_type == "taylor":
                    break
        ...
        relevant_axis.axis["top"].major_ticklabels.set(
            **plot_characteristics["rtick_params"]
        )
        relevant_axis.axis["top"].label.set_text(rlabel)
        relevant_axis.axis["top"].label.set_fontsize(
            plot_characteristics["rlabel"]["fontsize"]
        )
        ...
        relevant_axis.axis["right"].major_ticklabels.set_axis_direction(
            "bottom" if extend else "left"
        )
        relevant_axis.axis["right"].major_ticklabels.set(
            **plot_characteristics["xytick_params"]
        )

        # adjust left axis (x axis)
        relevant_axis.axis["left"].set_axis_direction("bottom")
        relevant_axis.axis["left"].major_ticklabels.set(
            **plot_characteristics["xytick_params"]
        )
        ...
        if extend:
            ...
            relevant_axis.axis["bottom"].label.set_fontsize(
                plot_characteristics["xylabel"]["fontsize"]
            )
        else:
            relevant_axis.axis["left"].label.set_text(xylabel)
            relevant_axis.axis["left"].label.set_fontsize(
                plot_characteristics["xylabel"]["fontsize"]
            )
        ...
        rms = np.sqrt(
            reference_stddev**2 + rs**2 - 2 * reference_stddev * rs * np.cos(ts)
        )
        contours = self.taylor_polar_relevant_axis.contour(
            ts,
            rs,
            rms,
            num_levels,
        ...
        # add contour labels
        self.taylor_polar_relevant_axis.clabel(
            contours, contours.levels, inline=True, fmt="%.2f", fontsize=6
        )
        ...
        self.taylor_polar_relevant_axis.plot(
            ref_x, ref_y, **plot_characteristics["contours"]["style"]["obs"]
        )
    ...
    def make_fairmode_target(
        ...
        RV, exc_threshold = get_fairmode_RV_exceendance(
            self.read_instance, speci, RV, exc_threshold, units
        )
        ...
        relevant_axis.text(
            **plot_characteristics["auxiliar"]["sides"]["top"],
            transform=relevant_axis.transAxes,
        )
        relevant_axis.text(
            **plot_characteristics["auxiliar"]["sides"]["bottom"],
            transform=relevant_axis.transAxes,
        )
        relevant_axis.text(
            **plot_characteristics["auxiliar"]["sides"]["left"],
            transform=relevant_axis.transAxes,
        )
        relevant_axis.text(
            **plot_characteristics["auxiliar"]["sides"]["right"],
            transform=relevant_axis.transAxes,
        )
        ...
        relevant_axis.plot(
            [xmin, xmax],
            [ymin, ymax],
            **plot_characteristics["auxiliar"]["crosses"]["increasing"],
        )
        relevant_axis.plot(
            [xmin, xmax],
            [ymax, ymin],
            **plot_characteristics["auxiliar"]["crosses"]["decreasing"],
        )
        ...
        for data_label in cut_data_labels:
            ...
            for x, y, classification in zip(x_points, y_points, classifications):
                ...
                stations_dots = relevant_axis.plot(
                    x,
                    y,
                    markeredgecolor=self.read_instance.plotting_params[data_label][
                        "colour"
                    ],
                    marker=marker,
                    **plot_characteristics["plot"],
            ...
            if "bad_stations" in plot_characteristics["annotate_options"]:
                if len(bad_stations) > 0:
                    formatted_stations = "\n".join(
                        station.replace("_", "") for station in bad_stations
                    )
                    self.faimode_target_annotate_text.append(f":\n{formatted_stations}")
                    self.faimode_target_annotate_colour.append("black")
        ...
        self.faimode_target_annotate_text[0] = self.faimode_target_annotate_text[
            0
        ].strip()
    ...
    def make_fairmode_statsummary(
        ...
        if (self.read_instance.mode not in ["report", "library"]) and (
            ...
            self.read_instance.handle_layout_update(
                "None",
                sender=self.canvas_instance.get_plot_type_position(
        ...
        for data_label in cut_data_labels:
            ...
            statistics_list = [
                np.array(means),
                np.array(exceedances),
                np.array(t_biases),
                np.array(t_R_list),
                np.array(t_sd_list),
                np.array(h_perc_list),
                np.nanmean(t_sd_list),
                np.nanmean(t_R_list),
            ]
            ...
            for i, (row, fairmode_data) in enumerate(zip(subplots, statistics_list)):
                ...
                if "range_style" in plot_dict:
                    ...
                    range_style_dict = plot_characteristics["auxiliar"]["range_style"][
                        plot_dict["range_style"]
                    ]
                    for span, color in zip(
                        range_style_dict["spans"], range_style_dict["colors"]
                    ):
                    ...
                    dot_color = plot_characteristics["auxiliar"]["right_dot_colors"][
                        "red"
                    ]
                    if len(arr) > 0 and len(correct_arr) / len(arr) >= 0.9:
                        dot_color = plot_characteristics["auxiliar"][
                            "right_dot_colors"
                        ]["green"]

                    # plot dot on the right
                    relevant_axis[i * 4 + 3].scatter(
                        **plot_characteristics["auxiliar"]["right_dot"],
                        color=dot_color,
                        edgecolor=dot_color,
                    )
                ...
                if np.any(right_zone_mask):
                    ...
                    stations_dots = relevant_axis[i * 4 + 2].plot(
                        ...
                        color=self.read_instance.plotting_params[data_label]["colour"],
                        markersize=plot_characteristics["auxiliar"]["station_dots"][
                            "markersize"
                        ],
                    )
                    ...
                    self.fairmode_statsummary_row_titles[data_label].append(
                        f"{row}_right"
                    )

                    # remove it from the data plotted in the middle zone
                    fairmode_data = (
                        fairmode_data[~right_zone_mask]
                ...
                if left_dashed_zone_linestyle != "none":
                    ...
                    left_dashed_zone_linestyle = (
                        left_dashed_zone_linestyle[0],
                        tuple(left_dashed_zone_linestyle[1]),
                    )
                ...
                for side in ["bottom", "top", "left"]:
                    relevant_axis[i * 4 + 0].spines[side].set_linestyle(
                        left_dashed_zone_linestyle
                    )
                ...
                relevant_axis[i * 4 + 1].tick_params(
                    **plot_characteristics["auxiliar"]["station_dots"]["tick_params"]
                )
        ...
        for i, (row, fairmode_data) in enumerate(zip(subplots, statistics_list)):
            ...
            if row == "observed_exceedances":
                exc_threshold = round(
                    exc_threshold, plot_characteristics["round_decimal_places"]["title"]
                )
    ...
    def make_contingencytable(
        ...
        if (
            "per_station"
            not in self.canvas_instance.selected_station_data[networkspeci]
        ):
        ...
        for data_label in cut_data_labels:
            ...
            model_data = self.canvas_instance.selected_station_data[networkspeci][
                "per_station"
            ][valid_data_labels.index(data_label), :, :]
            ...
            for station_ind in np.arange(n_stations):
                ...
                if speci in ["pm2p5", "pm10"]:
                    st_observations_data = (
                        pd.Series(st_observations_data)
                        .rolling(window=24, min_periods=18)
                        .mean()
                        .to_numpy()
                    )
                    st_model_data = (
                        pd.Series(st_model_data)
                        .rolling(window=24, min_periods=18)
                        .mean()
                        .to_numpy()
                    )
                ...
                contingency_table = ModBias.calculate_contingency_table(
                    st_observations_data,
                    st_model_data,
                    limits,
                    index_levels,
                ...
                if make_gerrity:
                    gerrity_score = round(
                        ModBias.calculate_gerrity_score(contingency_table), 2
                    )
                    current_lon = round(
                        self.canvas_instance.selected_station_metadata[networkspeci][
                            "longitude"
                    ...
                    if gerrity_row_limit is not None:
                        if (
                            station_ind > gerrity_row_limit - 1
                            and n_stations != gerrity_row_limit
                        ):
                            results.append(
                                {
                                    "Station reference": "...",
                ...
                # show contingency table (only available per station)
                    ...
                    results_df = pd.DataFrame(
                        contingency_table.table.values, index=levels, columns=levels
                    )
            ...
            if not results_df.empty:
                ...
                if not make_gerrity:
                    ...
                    cells_to_merge = [
                        (-1, col) for col in range(len(results_df.columns))
                    ]
                ...
                if "fontsize" in plot_characteristics:
                    ...
                    table.auto_set_column_width(
                        np.arange(-1, len(results_df.columns) + 1)
                    )

                # track plot elements
                if self.read_instance.mode not in ["report"]:
                    self.track_plot_elements(
                        data_label, "contingencytable", "plot", [table], bias=False
                    )

    def track_plot_elements(
        self, data_label, base_plot_type, element_type, plot_object, bias=False
    ):
        """
        ...
        if base_plot_type not in self.canvas_instance.plot_elements:
            self.canvas_instance.plot_elements[base_plot_type] = {
                "active": "absolute",
                "absolute": {},
            }
        ...
        if (
            ...
            not in self.canvas_instance.plot_elements[base_plot_type]
        ):
            self.canvas_instance.plot_elements[base_plot_type][
                plot_element_varname
            ] = {}
        ...
        if (base_plot_type in ["periodic", "periodic-violin"]) & (data_label != "ALL"):
            ...
            if isinstance(plot_object, dict):
                self.canvas_instance.plot_elements[base_plot_type][
                    plot_element_varname
                ][data_label][element_type] += plot_object["bodies"]
            # add list of lines
            elif isinstance(plot_object, list):
                self.canvas_instance.plot_elements[base_plot_type][
                    plot_element_varname
                ][data_label][element_type] += plot_object
        # boxplot plot specific elements
        elif (base_plot_type == "boxplot") & (data_label != "ALL"):
            # add lines for all boxplot elements
            self.canvas_instance.plot_elements[base_plot_type][plot_element_varname][
                data_label
            ][element_type] += plot_object["boxes"]
            self.canvas_instance.plot_elements[base_plot_type][plot_element_varname][
                data_label
            ][element_type] += plot_object["medians"]
            self.canvas_instance.plot_elements[base_plot_type][plot_element_varname][
                data_label
            ][element_type] += plot_object["whiskers"]
            self.canvas_instance.plot_elements[base_plot_type][plot_element_varname][
                data_label
            ][element_type] += plot_object["caps"]
            self.canvas_instance.plot_elements[base_plot_type][plot_element_varname][
                data_label
            ][element_type] += plot_object["fliers"]
            self.canvas_instance.plot_elements[base_plot_type][plot_element_varname][
                data_label
            ][element_type] += plot_object["means"]
        ...
        # all other plot elements
            ...
            self.canvas_instance.plot_elements[base_plot_type][plot_element_varname][
                data_label
            ][element_type] += plot_object

        # in dashboard set element visibility
        if self.read_instance.mode not in ["report", "library"]:
            if (
                data_label
                not in self.canvas_instance.plot_elements["data_labels_active"]
            ) & (data_label != "ALL"):
                if base_plot_type not in [
                    ...
                    for element in self.canvas_instance.plot_elements[base_plot_type][
                        plot_element_varname
