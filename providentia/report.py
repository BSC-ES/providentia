...
class Report:
        ...
        **kwargs : dict
        ...
        if "section" in self.commandline_arguments.keys():
            if self.commandline_arguments["section"] in self.all_sections:
                index = self.parent_section_names.index(
                    self.commandline_arguments["section"].split("·")[0]
                )
                self.parent_section_names = [self.parent_section_names[index]]
            else:
                error = (
                    "Error: Section {} does not exist in configuration file.".format(
    ...
    def run(self):
        ...
        for section_ind, section in enumerate(self.parent_section_names):
            ...
            plot_characteristics = yaml.safe_load(
                open(self.plot_characteristics_filename)
            )
            self.plot_characteristics_templates = expand_plot_characteristics(
                plot_characteristics, "report"
            )
            ...
            if Version(matplotlib.__version__) < Version("3.8"):
                if plot_type[:6] == "taylor":
                    if (plot_type in self.station_plots_to_make) or (
                        plot_type in self.summary_plots_to_make
                    ):
            ...
            self.plots_to_make.extend(
                x
                for x in self.station_plots_to_make
    ...
    def start_pdf(self, filename):
        ...
        if not os.path.exists(os.path.dirname(reports_path)):
            if "/" in reports_path:
                self.logger.info(
                    "Path {0} does not exist and it will be created.".format(
        ...
        with PdfPages(reports_path_temp) as pdf:
            ...
            if ("section" in self.commandline_arguments.keys()) and (
                "·" in self.commandline_arguments["section"]
            ):
            ...
            self.data_range_min_station = {
                networkspeci: np.inf for networkspeci in self.networkspecies
            }
            self.data_range_max_summary = {
                networkspeci: 0 for networkspeci in self.networkspecies
            }
            self.data_range_max_station = {
                networkspeci: 0 for networkspeci in self.networkspecies
            }
            self.stddev_max_summary = {
                networkspeci: 0 for networkspeci in self.networkspecies
            }
            self.stddev_max_station = {
                networkspeci: 0 for networkspeci in self.networkspecies
            }
            ...
            if (len(summary_plots_to_make) > 0) or (len(station_plots_to_make) > 0):
                self.make_plots_per_subsection(
                    summary_plots_to_make, station_plots_to_make
                )
            ...
            for networkspeci in self.networkspecies:
                ...
                for plot_type in list(self.plot_characteristics.keys()):
                    ...
                    if (self.report_summary) & (self.report_stations):
                        if plot_type in self.summary_pages:
                            if networkspeci in self.summary_pages[plot_type]:
                                paradigm_pages["summary"] = self.summary_pages[
                                    plot_type
                                ][networkspeci]
                            ...
                            if plot_type in self.station_pages:
                                if networkspeci in self.station_pages[plot_type]:
                                    for subsection in self.subsections:
                                        if (
                                            subsection
                                            in self.station_pages[plot_type][
                                                networkspeci
                                            ]
                                        ):
                                            paradigm_pages["station"].extend(
                    ...
                    for plotting_paradigm, relevant_pages in paradigm_pages.items():
                        ...
                        ) = self.get_relevant_axs_per_networkspeci_plot_type(
                            base_plot_type, relevant_pages
                        )
                        ...
                        relevant_axs = [
                            ax for ax in relevant_axs if ax not in axs_to_remove
                        ]
                        ...
                        if "cb" in list(self.plot_characteristics[plot_type].keys()):
                            ...
                            cb_axs = [
                                self.plot_dictionary[relevant_page]["cb_ax"]
                        ...
                        if (
                            ...
                            and (self.harmonise_summary)
                        ) or (
                            (plotting_paradigm == "station")
                            and (self.harmonise_stations)
                        ):
                            harmonise = True
        ...
        if self.compression:
            self.logger.info("\nCompressing PDF\n")
            os.system(
                "gs -sDEVICE=pdfwrite -dCompatibilityLevel=1.4 -dPDFSETTINGS=/printer -dNOPAUSE -dQUIET -dBATCH -sOutputFile={} {}".format(
        ...
        if "actris/actris" in self.network:
            ...
            doi_pdf = self.plotting.make_doi_pdf(
                self.plot_characteristics["doi"], reports_doi_path_temp
            )
        ...
        self.reorder_pdf_pages(
            reports_path,
            reports_path,
            self.summary_multispecies_pages,
            self.station_multispecies_pages,
            self.paradigm_break_page,
            doi_pdf,
            reports_doi_path_temp,
        )

    def setup_plot_geometry(
        self, plotting_paradigm, networkspeci, have_setup_multispecies
    ):
        """
        ...
        for plot_type in plots_to_make:
            ...
            if plotting_paradigm == "summary":
                if "multispecies" in plot_options:
                    plot_characteristics["page_title"][
                        "t"
                    ] = "{} (Summary)\nmultispecies".format(
                        plot_characteristics["page_title"]["t"]
                    )
                else:
                    plot_characteristics["page_title"]["t"] = "{} (Summary)\n{}".format(
                        plot_characteristics["page_title"]["t"], networkspeci
                    )
            elif plotting_paradigm == "station":
                if "multispecies" in plot_options:
                    plot_characteristics["page_title"][
                        ...
                        plot_characteristics["page_title"]["t"], self.subsection
                    )
                else:
                    plot_characteristics["page_title"][
                        "t"
            ...
            if base_plot_type == "map":
                if "obs" in plot_options:
                    ...
                    n_plots_per_plot_type = len(self.subsections) * len(
                        self.data_labels
                    )
                ...
                if plotting_paradigm == "summary":
                    ...
                    if "individual" in plot_options:
                        if (
                            ...
                            n_plots_per_plot_type = self.n_stations * len(
                                self.data_labels
                            )
                    else:
                        n_plots_per_plot_type = self.n_stations
            ...
            n_pages_per_plot_type = int(
                np.ceil(
                    n_plots_per_plot_type
                    / (
                        plot_characteristics["figure"]["ncols"]
                        * plot_characteristics["figure"]["nrows"]
                    )
                )
            )
            plot_ii_per_type = 0
            ...
            for page_n in range(
                self.n_total_pages, self.n_total_pages + n_pages_per_plot_type
            ):
                if base_plot_type == "map":
                    plot_characteristics["figure"]["subplot_kw"] = {
                        "projection": self.plotcrs
                    }
                elif base_plot_type == "taylor":
                    reference_stddev = 7.5
                    ghelper = get_taylor_diagram_ghelper(
                        reference_stddev, plot_characteristics
                    )
                    plot_characteristics["figure"]["subplot_kw"] = {
                ...
                for ax_ii, ax in enumerate(axs.flatten()):
                    ...
                    if plot_ii_per_type < n_plots_per_plot_type:
                        ...
                        if (
                            n_plots_per_plot_type - plot_ii_per_type
                        ) <= plot_characteristics["figure"]["ncols"]:
                            last_valid_row = True
                        else:
                        ...
                        if base_plot_type in ["periodic", "periodic-violin"]:
                            ...
                            ) in self.periodic_relevant_temporal_resolutions:
                                annotations = [
                                    child
                                    for child in grid_dict[
                                        relevant_temporal_resolution
                                    ].get_children()
                        ...
                        # setup fairmode summary stats plot type gridspec
                            ...
                            gs = gridspec.GridSpec(
                                nrows, ncols, **plot_characteristics["gridspec_kw"]
                            )
                            grid_list = [
                ...
                if "subplots_adjust" in plot_characteristics_vars:
                    if base_plot_type == "contingencytable":
                        if "gerrity" in plot_options:
                            fig.subplots_adjust(
                                **plot_characteristics["subplots_adjust"]["gerrity"]
                            )
                        else:
                            fig.subplots_adjust(
                                **plot_characteristics["subplots_adjust"]["contingency"]
                            )
                    else:
                ...
                if "legend" in plot_characteristics_vars:
                    if (
                        (base_plot_type in ["scatter", "fairmode-target"])
                        or ("bias" in plot_options)
                        or (z_statistic_sign == "bias")
                    ):
                        set_obs = False
                        ...
                        set_obs = True
                    plot_characteristics["legend"] = self.plotting.make_legend_handles(
                        plot_characteristics["legend"], set_obs=set_obs
                    )
                ...
                if "cb" in plot_characteristics_vars:
                    if "cb_ax" not in self.plot_dictionary[page_n]:
                        self.plot_dictionary[page_n]["cb_ax"] = fig.add_axes(
                            plot_characteristics["cb"]["position"]
                        )
                ...
                if plotting_paradigm == "summary":
                    ...
                    self.station_pages[plot_type][networkspeci][self.subsection].append(
                        page_n
                    )
    ...
    def make_plots_per_subsection(
        self, summary_plots_to_make, station_plots_to_make, do_plot_geometry_setup=False
    ):
        """
    ...
    def reorder_pdf_pages(
        ...
        if (len(self.summary_multispecies_pages) > 0) or (
            len(self.station_multispecies_pages) > 0
        ):
            self.logger.info("\nReordering pages")
        ...
        if len(summary_multispecies_pages) > 0:
            summary_multispecies_pages = np.concatenate(
                (np.array([0]), summary_multispecies_pages)
            )
            summary_other_plots_pages = all_pages[
                ~np.isin(all_pages, summary_multispecies_pages)
            ]
            page_order = np.concatenate(
                (summary_multispecies_pages, summary_other_plots_pages)
            ).tolist()
        ...
        if len(station_multispecies_pages) > 0:
            ...
            station_other_plots_pages = station_pages[
                ~np.isin(station_pages, station_multispecies_pages)
            ]
            page_order = np.concatenate(
                (
                    page_order[:paradigm_break_page],
                    station_multispecies_pages,
                    station_other_plots_pages,
                )
            ).tolist()
        ...
        for month_metadata in self.selected_station_metadata[networkspeci]:
            for metadata_vars in month_metadata:
                ...
                for var in metadata_vars:
                    if isinstance(var, str) and re.compile(
                        r"^https?://doi\.org/"
                    ).match(var):
    ...
    def make_summary_plots(self, networkspeci, summary_plots_to_make):
        ...
        if (not self.summary_plot_geometry_setup) & (self.do_plot_geometry_setup):
            self.setup_plot_geometry(
                "summary", networkspeci, self.made_networkspeci_summary_plots
            )

        # get valid station inds for networkspeci
        if self.temporal_colocation:
            self.relevant_station_inds = self.valid_station_inds_temporal_colocation[
                networkspeci
            ][self.observations_data_label]
        else:
            self.relevant_station_inds = self.valid_station_inds[networkspeci][
                self.observations_data_label
            ]
        ...
        if self.n_stations == 0:
            self.logger.error(
                "\nNo valid stations for {}, {}. Not making summmary plots".format(
                    networkspeci, self.subsection
                )
            )
        else:
            self.logger.info(
        ...
        if not self.made_networkspeci_summary_plots:
            ...
            for ns in self.networkspecies:
                if self.selected_station_data_min[ns] < self.data_range_min_summary[ns]:
                    self.data_range_min_summary[ns] = copy.deepcopy(
                        self.selected_station_data_min[ns]
                    )
                if self.selected_station_data_max[ns] > self.data_range_max_summary[ns]:
                    self.data_range_max_summary[ns] = copy.deepcopy(
                        self.selected_station_data_max[ns]
                    )
        ...
        if networkspeci.split("|")[0] == "actris/actris" and not have_nodata:
            for relevant_station_ind in self.relevant_station_inds:
                self.current_station_reference = self.station_references[networkspeci][
                    relevant_station_ind
                ]
                self.station_reference_data[self.subsection][networkspeci].append(
                    self.current_station_reference
                )
                current_station_dois = self.get_dois_from_station_reference(
                    self.current_station_reference, networkspeci
                )
                self.station_doi_data[self.subsection][networkspeci].append(
                    current_station_dois
                )
        ...
        for plot_type in summary_plots_to_make:
            ...
            if (
                (not plot_type_df)
                & ("multispecies" not in plot_options)
                & (have_nodata)
            ):
                continue
            ...
            if base_plot_type in ["fairmode-target", "fairmode-statsummary"]:
                ...
                if not any(valid_station_idxs):
                    self.logger.info(
                        f"No data to generate FAIRMODE plot after filtering by coverage for {speci}."
                    )
                    continue
            ...
            # make plot
            plot_indices = self.make_plot(
                "summary", plot_type, plot_options, networkspeci
            )
            ...
            (
                relevant_axs,
                relevant_data_labels,
            ) = self.get_relevant_axs_per_networkspeci_plot_type_page_ind(
    ...
    def make_station_plots(self, networkspeci, station_plots_to_make):
        ...
        if self.do_plot_geometry_setup:
            self.setup_plot_geometry(
                "station", networkspeci, self.made_networkspeci_station_plots
            )
        ...
        if self.n_stations == 0:
            self.logger.info(
                "No valid stations for {}, {}. Not making station plots".format(
                    networkspeci, self.subsection
                )
            )
        else:
            self.logger.info(
                "Making {}, {} station plots".format(networkspeci, self.subsection)
            )
        ...
        for i, relevant_station_ind in enumerate(self.relevant_station_inds):
            ...
            self.station_ind += 1
            self.current_lon = round(
                self.station_longitudes[networkspeci][relevant_station_ind], 2
            )
            self.current_lat = round(
                self.station_latitudes[networkspeci][relevant_station_ind], 2
            )
            self.current_station_name = self.station_names[networkspeci][
                relevant_station_ind
            ]
            self.current_station_reference = self.station_references[networkspeci][
                relevant_station_ind
            ...
            if (
                self.selected_station_data_min[networkspeci]
                < self.data_range_min_station[networkspeci]
            ):
                self.data_range_min_station[networkspeci] = copy.deepcopy(
                    self.selected_station_data_min[networkspeci]
                )
            if (
                self.selected_station_data_max[networkspeci]
                > self.data_range_max_station[networkspeci]
            ):
            ...
            if (
                ...
                self.stddev_max_station[networkspeci] = copy.deepcopy(
                    self.selected_station_stddev_max[networkspeci]
                )
            ...
            for plot_type in station_plots_to_make:
                ...
                if (
                    ...
                    & (self.reading_ghost)
                    & ("_" in self.current_station_reference)
                ):
                    self.current_station_reference = "_".join(
                        self.current_station_reference.split("_")[:-1]
                    )
                ...
                if "multispecies" in plot_options:
                    ...
                    if not plot_type_df:
                        ...
                        for ns in self.networkspecies:
                            if (
                                self.selected_station_data_min[ns]
                                < self.data_range_min_station[ns]
                            ):
                                self.data_range_min_station[ns] = copy.deepcopy(
                                    self.selected_station_data_min[ns]
                                )
                            if (
                                self.selected_station_data_max[ns]
                                > self.data_range_max_station[ns]
                            ):
                                self.data_range_max_station[ns] = copy.deepcopy(
                                    self.selected_station_data_max[ns]
                                )
                            if (
                                self.selected_station_stddev_max[ns]
                                > self.stddev_max_station[ns]
                            ):
                                self.stddev_max_station[ns] = copy.deepcopy(
                                    self.selected_station_stddev_max[ns]
                                )
                ...
                self.logger.info(
                    "\nMaking station {2} for {3} ({0}/{1})".format(
                        i + 1,
                ...
                if base_plot_type in ["fairmode-target", "fairmode-statsummary"]:
                    ...
                    if not any(valid_station_idxs):
                        self.logger.info(
                            f"No data to generate FAIRMODE plot after filtering by coverage for {speci} in {self.current_station_name}."
                        )
                        continue
                ...
                # make plot
                plot_indices = self.make_plot(
                    "station", plot_type, plot_options, networkspeci
                )
    ...
    def update_stats_tables(
        ...
        for data_label in self.data_labels:
            ...
            for stat in stats:
                ...
                if (data_label == self.observations_data_label) & (
                ...
                if plotting_paradigm == "summary":
                    ...
                    if (
                        self.current_station_reference
                        not in self.stats_station[self.subsection][networkspeci]
                    ):
                        self.stats_station[self.subsection][networkspeci][
                            self.current_station_reference
                        ] = {}
                ...
                if data_label in self.selected_station_data_labels[networkspeci]:
                    ...
                    if (
                        ...
                        and (stat in self.modbias_stats)
                        and (
                    ...
                    # otherwise calculate statistic
                        ...
                        if z_statistic_sign == "bias":
                            data_to_add = calculate_statistic(
                                self,
                                self,
                                networkspeci,
                                zstat,
                            ...
                            data_to_add = calculate_statistic(
                                self, self, networkspeci, zstat, [data_label], []
                            )
                else:
                    data_to_add = np.nan
    ...
    def make_plot(self, plotting_paradigm, plot_type, plot_options, networkspeci):
        ...
        if (
            (("bias" in plot_options) or (z_statistic_sign == "bias"))
            & (len(data_labels) < 2)
            & (not plot_type_df)
        ):
        ...
        if base_plot_type == "contingencytable":
            if (not self.temporal_colocation) or (
                ...
                and (len(data_labels) == 1 or len(data_labels) > 2)
            ):
        ...
        plot_validity = False
        if (
            (base_plot_type in ["scatter", "fairmode-target", "fairmode-statsummary"])
            or ("bias" in plot_options)
            or (z_statistic_sign == "bias")
        ):
        ...
        if base_plot_type == "map":
            ...
            for z1_label, z2_label in zip(z1, z2):
                ...
                skip_map = False
                if (z2_label == "") and (
                    z1_label not in self.selected_station_data_labels[networkspeci]
                ):
                    skip_map = True
                elif (z2_label != "") and (
                    (z1_label not in self.selected_station_data_labels[networkspeci])
                ...
                axis_ind = (
                    current_plot_ind * len(self.subsections)
                ) + self.subsection_ind
                relevant_page, page_ind, relevant_axis = self.get_relevant_page_axis(
                    plotting_paradigm, networkspeci, plot_type, axis_ind
                )
                ...
                if z2_label == "":
                    self.plot_dictionary[relevant_page]["axs"][page_ind][
                        "data_labels"
                    ].append(z1_label)
                else:
                    self.plot_dictionary[relevant_page]["axs"][page_ind][
        ...
        # other plots (1 plot per subsection with multiple data arrays for summary paradigm, 1 plot per subsection per station for station paradigm)
            ...
            if base_plot_type == "timeseries":
                if zstat:
                    ...
                    if chunk_resolution not in available_timeseries_chunk_resolutions:
                        msg = (
                            f"{plot_type} cannot be created because {chunk_resolution} "
                        )
            ...
            for data_labels in iter_data_labels:
                # skip observations
                if (data_labels == self.observations_data_label) and (
                    (
                        base_plot_type
                ...
                if "individual" in plot_options:
                    if (
                        data_labels
                        not in self.selected_station_data_labels[networkspeci]
                    ):
                        msg = f"{plot_type} cannot be created because there is no available data."
                        show_message(self, msg)
                        return plot_indices
                    ...
                    if "individual" in plot_options:
                        if (
                            ...
                            axis_ind = (
                                current_plot_ind
                                + self.subsection_ind
                                + (len(self.experiments) - 1) * self.subsection_ind
                            )
                        else:
                            axis_ind = (
                                current_plot_ind
                                + self.subsection_ind
                                + len(self.experiments) * self.subsection_ind
                            )
                    else:
                    ...
                    if "individual" in plot_options:
                        if (
                            ...
                            axis_ind = (
                                current_plot_ind
                                + self.station_ind
                                + (len(self.experiments) - 1) * self.station_ind
                            )
                        else:
                            axis_ind = (
                                current_plot_ind
                                + self.station_ind
                                + len(self.experiments) * self.station_ind
                            )
                    else:
                    ...
                    if "xlabel" in self.plot_characteristics[plot_type]:
                        ...
                        if "[measurement_units]" in xlabel:
                            xlabel = xlabel.replace(
                                "[measurement_units]",
                                "[{}]".format(
                                    self.measurement_units[networkspeci.split("|")[-1]]
                                ),
                            )
                    else:
                        xlabel = ""
                ...
                if (axis_ylabel == "") or ("[measurement_units]" in axis_ylabel):
                    if base_plot_type in ["periodic"] or (
                        (base_plot_type == "timeseries")
                        ...
                        if ylabel_units == "[measurement_units]":
                            ylabel_units = self.measurement_units[
                                networkspeci.split("|")[-1]
                            ]
                        if ylabel_units != "":
                ...
                if base_plot_type in ["fairmode-target", "fairmode-statsummary"]:
                    func = getattr(
                        self.plotting,
                        "make_{}".format(base_plot_type.replace("-", "_")),
                    )
                else:
                    func = getattr(
                ...
                if axis_title == "":
                    if plotting_paradigm == "summary":
                        if self.n_stations == 1:
                            axis_title_label = "{} ({} station)".format(
                                self.subsection, self.n_stations
                            )
                        else:
                            axis_title_label = "{} ({} stations)".format(
                                self.subsection, self.n_stations
                            )
                    elif plotting_paradigm == "station":
        ...
        # statsummary is multiple stats per networkspecies, subsections and data labels (columns are stats, rows are networkspecies / subsections / data labels)
            ...
            relevant_page, page_ind, relevant_axis = self.get_relevant_page_axis(
                plotting_paradigm, axis_networkspeci, plot_type, axis_ind
            )
            ...
            if base_plot_type in ["heatmap", "table"]:
                ...
                stats_df = pd.DataFrame(
                    np.nan, index=index, columns=data_labels, dtype=np.float64
                )
                            ...
                            if networkspeci in stats_to_plot[subsection]:
                                if plotting_paradigm == "summary":
                                    if zstat in stats_to_plot[subsection][networkspeci]:
                                        if (
                                            data_label
                                            in stats_to_plot[subsection][networkspeci][
                                                zstat
                                            ]
                                        ):
                                            stat_to_append = stats_to_plot[subsection][
                                                networkspeci
                                            ][zstat][data_label]
                                elif plotting_paradigm == "station":
                                    if (
                                        ...
                                        if (
                                            ...
                                            if (
                                                data_label
                                                in stats_to_plot[subsection][
                                                    networkspeci
                                                ][self.current_station_reference][zstat]
                                            ):
                                                stat_to_append = stats_to_plot[
                                                    subsection
                                                ][networkspeci][
                                                    self.current_station_reference
                                                ][
                ...
                # create empty dataframe with networkspecies and subsections
                index = pd.MultiIndex.from_product(
                    [self.networkspecies, self.subsections, data_labels],
                    names=["networkspecies", "subsections", "labels"],
                )
                stats_df = pd.DataFrame(
                    np.nan, index=index, columns=stats, dtype=np.float64
                )
                ...
                for subsection in subsections:
                    for networkspeci in networkspecies:
                        for data_label in data_labels:
                            ...
                            stats_df.loc[
                                (networkspeci, subsection, data_label)
                            ] = stats_per_data_label
            ...
            # if have no data then do not make plot
                ...
                msg = (
                    f"{plot_type} cannot be created because there is no available data."
                )
                show_message(self, msg)
                return plot_indices
    ...
    def get_relevant_page_axis(
        self, plotting_paradigm, networkspeci, plot_type, axis_ind
    ):
        """
        ...
        if plotting_paradigm == "summary":
            ...
            relevant_pages = self.station_pages[plot_type][networkspeci][
                self.subsection
            ]
        ...
        for relevant_page in relevant_pages:
            ...
            all_relevant_pages.extend(
                [relevant_page] * len(self.plot_dictionary[relevant_page]["axs"])
            )
            page_inds.extend(
                list(range(len(self.plot_dictionary[relevant_page]["axs"])))
            )

        return (
            all_relevant_pages[axis_ind],
            page_inds[axis_ind],
            relevant_axes[axis_ind]["handle"],
        )

    def get_relevant_axs_per_networkspeci_plot_type(
        self, base_plot_type, relevant_pages
    ):
        """
    ...
    def get_relevant_axs_per_networkspeci_plot_type_page_ind(
        self, base_plot_type, plot_indices
    ):
        """
        ...
        for relevant_page, page_ind in plot_indices:
            if base_plot_type in ["periodic", "periodic-violin"]:
                for (
                    ...
                    relevant_axs.append(
                        self.plot_dictionary[relevant_page]["axs"][page_ind]["handle"][
                            relevant_temporal_resolution
                        ]
                    )
                    relevant_data_labels.append(
                        self.plot_dictionary[relevant_page]["axs"][page_ind][
                            "data_labels"
                        ]
                    )
                ...
                relevant_data_labels.append(
                    self.plot_dictionary[relevant_page]["axs"][page_ind]["data_labels"]
                )
