...
class PopUpWindow(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        for nested_window_n in range(n_nested_windows):
            ...
            for label_idx, val in enumerate(self.checkbox_labels[nested_window_n]):
                ...
                if label_idx in current_selected_indices:
                    self.checkboxes[nested_window_n][label_idx].setCheckState(
                        QtCore.Qt.Checked)
                checkbox_grid.addWidget(self.checkboxes[nested_window_n][label_idx],
                                        row_n, column_n)
                # checkbox_grid.addWidget(self.checkboxes[nested_window_n][label_idx])
                row_n += 1
        ...
        quit_event.triggered.connect(self.close_event)

    def select_all(self, nested_window_n):
        """function to select all checkboxes"""
        for checkbox_idx, _ in enumerate(self.checkboxes[nested_window_n]):
            self.checkboxes[nested_window_n][checkbox_idx].setCheckState(
                QtCore.Qt.Checked)

    def clear_all(self, nested_window_n):
        """function to clear all checkboxes"""
        for checkbox_idx, _ in enumerate(self.checkboxes[nested_window_n]):
            self.checkboxes[nested_window_n][checkbox_idx].setCheckState(QtCore.Qt.Unchecked)
    ...
    def select_all_default(self, nested_window_n):
        ...
        for checkbox_idx in self.default_checkbox_selection[nested_window_n]:
            self.checkboxes[nested_window_n][checkbox_idx].setCheckState(QtCore.Qt.Checked)
    ...
    def close_event(self, event):
        ...
        for checkbox_idx in range(len(self.checkboxes)):
            checked_indices = np.array([], dtype=np.uint8)
            for checkbox_idx2 in range(len(self.checkboxes[checkbox_idx])):
                if self.checkboxes[checkbox_idx][checkbox_idx2].checkState() == QtCore.Qt.Checked:
                    checked_indices = np.append(checked_indices, checkbox_idx2)
...
class GenerateProvidentiaDashboard(QtWidgets.QWidget, ProvConfiguration):
    ...
    def init_ui(self):
        ...
        # create parent layout to pull together a configuration bar, a MPL
        # navigation toolbar, and a MPL canvas of plots
        ...
        self.flag_default_codes = np.array([1, 2, 3, 10, 11, 12, 13, 14, 15,
                                            16, 20, 21, 24, 25, 26, 29, 30,
                                            31, 32, 40, 41, 42, 43, 44, 45,
                                            46, 47, 48, 50, 51, 52, 53, 54,
                                            55, 70, 71, 72, 73, 74, 75, 76,
                                            77, 78, 79, 80, 81, 90, 150, 154,
                                            155, 156, 157], dtype=np.uint8)
        self.flag_default_inds = np.array([np.where(self.flag_codes ==
                                                    code)[0][0] for code in
                                           self.flag_default_codes],
                                          dtype=np.uint8)

        # define qa flags
        self.standard_qa_flag_codes = \
            json.load(open('providentia/conf/standard_qa_flag_codes.json'))
        ...
        self.qa_default_codes = np.array([0, 1, 2, 3, 4, 5, 70, 80, 81, 82,
                                          85, 87, 90, 94, 100, 106, 107, 108],
                                         dtype=np.uint8)
        self.qa_default_inds  = np.array([np.where(self.qa_codes == code)[0][0]
                                          for code in self.qa_default_codes],
                                         dtype=np.uint8)
        ...
        self.class_default_codes_to_retain = np.array([5], dtype=np.uint8)
        self.class_default_codes_to_remove = np.array([0, 4, 46, 49], dtype=np.uint8)
        self.class_default_inds_to_retain = \
            np.array([np.where(self.classification_codes == code)[0][0] for code
                      in self.class_default_codes_to_retain],
                     dtype=np.uint8)
        self.class_default_inds_to_remove = \
            np.array([np.where(self.classification_codes == code)[0][0] for code
                      in self.class_default_codes_to_remove],
                     dtype=np.uint8)
    ...
    def update_configuration_bar_fields(self):
        ...
        if len(self.selected_indices['EXPERIMENTS'][0]) > 0:
            previous_selected_experiments = \
                self.previous_avail_exp_grids[self.selected_indices['EXPERIMENTS'][0]]
        else:
            previous_selected_experiments = []
        ...
        selected_experiments = [previous_selected_experiment for previous_selected_experiment
                                ...
                                in self.avail_exp_grids]
        selected_experiment_inds = \
            np.array([np.where(self.avail_exp_grids == selected_experiment)[0][0]
        ...
        self.previous_avail_exp_grids = np.array(self.avail_exp_grids)
            ...
            for grid in available_grids:
                ...
                if not os.path.exists('%s/%s/%s/%s/%s/%s/%s'%(exp_root,GHOST_version,experiment,grid,self.selected_resolution,self.selected_species,self.selected_network)):
                    ...
                    if len(valid_network_files_yearmonths) > 0:
                        self.avail_exp_data['%s-%s'%(experiment,grid)] = valid_network_files_yearmonths

        # get list of available experiment-grid names
        self.avail_exp_grids = np.array(sorted(list(self.avail_exp_data.keys())))
    ...
    def handle_pop_up_experiments_window(self):
        # setup pop up window
        self.experiments_window = PopUpWindow(window_type='EXPERIMENTS', window_titles=['Select Experiment/s'], checkbox_labels=[self.avail_exp_grids], default_checkbox_selection=[[]], selected_indices=self.selected_indices)

    def handle_pop_up_flags_window(self):
        # setup pop up window
        self.qa_window = PopUpWindow(window_type='FLAGS', window_titles=['Select standardised data reporter provided flags to filter by'], checkbox_labels=[self.flag_names], default_checkbox_selection=[self.flag_default_inds], selected_indices=self.selected_indices)

    def handle_pop_up_qa_window(self):
        # setup pop up window
        self.qa_window = PopUpWindow(window_type='QA', window_titles=['Select standardised QA flags to filter by'], checkbox_labels=[self.qa_names], default_checkbox_selection=[self.qa_default_inds], selected_indices=self.selected_indices)

    def handle_pop_up_classifications_window(self):
        # setup pop up window
        self.qa_window = PopUpWindow(window_type='CLASSIFICATIONS', window_titles=['Select standardised classifications to retain','Select standardised classifications to remove'], checkbox_labels=[self.classification_names,self.classification_names], default_checkbox_selection=[self.class_default_inds_to_retain,self.class_default_inds_to_remove], selected_indices=self.selected_indices)

    def handle_pop_up_methods_window(self):
        # only proceed if have some valid stations in memory
        if len(self.station_references) > 0:
            # setup pop up window
            self.qa_window = PopUpWindow(window_type='METHODS', window_titles=['Select standardised measurement methodologies to retain'], checkbox_labels=[self.station_unique_methods], default_checkbox_selection=[np.arange(len(self.station_unique_methods), dtype=np.int)], selected_indices=self.selected_indices)
    ...
    def handle_data_selection_update(self):
        ...
        if len(self.selected_indices['EXPERIMENTS'][0]) > 0:
            self.active_experiment_grids = self.avail_exp_grids[self.selected_indices['EXPERIMENTS'][0]]
        else:
            self.active_experiment_grids = []
        ...
        # generate list of sorted z1/z2 data arrays names in memory, putting
        # observations before experiments and empty string item as first
        # element in z2 array list (for changing from 'difference' statistics
        # to 'absolute')
    ...
    def read_setup(self):
        ...
        # ------------------------------------------------------------------------------------# 
        # iterate through all relevant observational files and read station references/longitudes/latitudes
    ...
    def read_data(self, data_label, start_date_to_read, end_date_to_read):
        ...
        if data_label == 'observations':
            ...
            relevant_file_start_dates = sorted(self.avail_exp_data[data_label])
